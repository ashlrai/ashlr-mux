...
- The goal remains incomplete. `ws_pty_windows.go` still exposes a compile-compatible Windows adapter whose runtime server returns `websocket PTY transport is not implemented on Windows in M0`; implementing canonical remote WebSocket PTY behavior on Windows is a concrete remaining parity gap after this sync is pushed.
