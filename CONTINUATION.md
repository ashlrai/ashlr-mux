...
- Canonical no-socket `cmux welcome` is implemented and ready to push: Windows prints the complete canonical light-theme true-color ANSI logo, tagline, 12 shortcut rows, Docs/Discord/GitHub/Email links, and command hints with exact fixed visual columns and leading/trailing blank lines. The red local-dispatch test, renderer content/ANSI test, fixed-column red regression, shared row-appender simplification, rebuilt executable retest, all 151 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
