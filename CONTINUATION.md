...
- Canonical atomic `reorder-workspaces` / `workspace.reorder_many` is fully implemented and verified, ready to push: exact `--order` presence/empty-item errors; comma-separated numeric/ref/id normalization; optional window scope; `--dry-run`; canonical applied/plan text; and concrete help. The desktop advertises the method, preserves request order while resolving refs/UUIDs, validates duplicates before unknown workspaces, returns canonical error and plan/events payloads, and applies the shared golden-pinned batch planner atomically under one session lock. The new core application rebuilds requested leading order inside pinned/unpinned tiers, keeps unmentioned peers stable, restores group contiguity/anchor order, follows selection, and leaves state unchanged for dry runs or validation errors. Dry-run response refs come from a fully planned clone without mutating authoritative state. The red route/output/help tests, two executable core mutation/group/atomicity tests, compiled desktop parser test, real built-executable named-pipe batch regression, two simplification/retest cycles, all 265 core tests, all 87 config tests, all 191 CLI library + 17 binary + 13 executable-regression tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Next after push: audit canonical `reorder-surface`; it is advertised on Windows but has neither a top-level CLI route nor a `surface.reorder` backend method. Determine the pane/tab ordering model and implement only after pinning selector, cross-pane, focus, and response semantics. The overall parity goal is not complete.
