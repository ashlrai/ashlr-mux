...
- Kiro hook installation is implemented and ready to push: `cmux hooks kiro install` and setup aliases write the canonical custom-agent `agents/cmux.json`, default fresh agents to `tools: ["*"]`, preserve user fields/tools/hooks, replace only cmux-owned entries idempotently, install 5-second lifecycle hooks and 120-second pre/post-tool Feed hooks, omit Cursor's `version`, and print the required `kiro-cli chat --agent cmux` activation guidance. Red parser/shape tests, the owned-entry/idempotence simplification, isolated executable install/reinstall smoke, all 124 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
