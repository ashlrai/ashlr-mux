...
- Feed OpenTUI launch parity is implemented and ready to push: `cmux feed tui` / `--opentui` now parse canonical flags, require an interactive terminal before side effects, discover Bun from override/PATH/home, embed and stage the canonical TypeScript source plus pinned `@opentui/core` 0.1.106 package, install only on version drift, resolve the shared socket/password contract, and launch with canonical alternate-screen environment and exit handling. Nested TUI help and errors are exact; `--legacy` and automatic fallback remain the next slice. Red parser/plan tests, the shared socket-resolution simplification, embedded-source Bun bundling, executable help/noninteractive/no-staging/unknown smokes, all 121 CLI library + 13 binary tests, CLI build, strict Clippy with only the three known unrelated allowances, desktop all-target checking/test linking/build, all 1,337 desktop/webview tests, web typecheck/build, formatting, and whitespace checks pass.
