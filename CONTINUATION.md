...
- Marker-owned Pi/OMP/Amp hook uninstall is implemented and ready to push: direct `hooks AGENT uninstall` removes only a marker-owned extension/plugin, reports already-missing files idempotently, refuses while preserving an unmarked user file, and is advertised by concrete hooks help. All three agents share one read/ownership/remove state machine, and Amp install/uninstall now share one config resolver. The red parser/ownership test, isolated executable install/remove/missing/refusal smokes, simplification review, all 138 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
