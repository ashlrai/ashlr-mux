...
- Feed beta availability and shell parity is implemented and ready to push: the canonical-off-by-default preference persists in the existing app settings store, Settings exposes the Feed toggle, native mode validation admits Feed only while enabled and falls back to Files when disabled, and the mode bar plus command palette update dynamically. The Feed panel now has canonical Actionable/All Activity controls and empty states, establishing the real host for the next `feed.push`/items/replies slice. Focused red/green tests, the state-read simplification, all 1,329 desktop/webview tests, web typecheck/build, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
