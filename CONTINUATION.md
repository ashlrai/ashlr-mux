...
- Nested-JSON hook uninstall for Gemini, Grok, Copilot, CodeBuddy, Factory, and Qoder is implemented and ready to push: direct uninstall prunes owned lifecycle, Feed, and stale-event commands across the whole hooks object, drops only emptied groups/events, preserves mixed user commands, unknown group shapes, scalar hooks, and unrelated root fields, and treats missing/invalid configs as canonical no-ops. Install shares the same in-place `retain_mut` ownership pruning, and token-boundary executable matching avoids deleting user commands that merely mention `cmux hooks`. The red parser/mixed-group/stale-event/false-positive tests, isolated executable install/uninstall/preservation smokes for all six agents plus invalid JSON, simplified Qoder retest, all 140 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
