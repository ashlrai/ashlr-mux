...
- Canonical `config reload` socket alias is ready to push: it routes case-insensitively to the existing `config.reload` control method, rejects extra arguments with exact canonical usage, and deliberately leaves `config set` for its local-write mutation executor. The red dispatch test, arity/set-isolation simplification guards, focused retest, executable missing-socket routing/arity smoke, all 86 shared-config tests, all 160 CLI library + 14 binary tests, strict config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
