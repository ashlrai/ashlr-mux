...
- Window introspection parity is implemented and ready to push: `list-windows` and `current-window` now call read-only `window.list` / `window.current` backend methods projected from the Windows session snapshot, preserve canonical JSON shapes, canonical selected/workspace-count plain text, and concrete help. Focused red/green tests, a direct snapshot-projection simplification pass, the complete `cmux-cli` suite (99 library + 9 binary tests), both help probes, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
