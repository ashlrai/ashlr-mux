...
- `rename-window` compatibility parity is implemented and ready to push: the canonical alias now reuses `workspace.rename`, the existing workspace selector/title parser, ambient workspace scope, and concrete help while preserving its alias-specific missing-title error. Focused red/green tests, a simplification pass that kept one shared rename parser, the complete `cmux-cli` suite (99 library + 4 binary tests), help smoke output, formatting, and whitespace checks pass.
