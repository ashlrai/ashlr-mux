...
- The parity program now uses a frozen, generated, independently verified matrix baseline. Canonical is pinned to `e1825d40d52b4ae4f4bcb0b7e0dfc744dd20a452` (2026-07-09) and Windows evidence to `689854602b58dd1f6ee24f3d6a94182cfa8c7423`; upstream movement no longer silently changes the target. Deterministic extractors catalog 174 accepted CLI tokens (158 public + 16 hidden), 293 v2 methods (251 release + 42 debug), 19 product/runtime requirements, and exact Windows route/help/unsupported evidence. The joined matrix has 486 unique entries: 233 `implemented_unverified`, 253 `missing`, zero resolved, strict parity `0/486 (0.0%)`, completion `0/486 (0.0%)`, and secondary core-local workflow `0/277 (0.0%)`. Source presence is capped at `implemented_unverified`; 12 explicit `not_supported` routes remain missing. The first independent audit failed five invalid product paths, 21 dangling alias dependencies, and six undercounted conditional/helper CLI routes; all were fixed, dependency/source validation was made exhaustive, and the fresh verifier PASS at `446746eaf` confirms 207 pinned blobs, no dangling IDs, schema/count/status arithmetic, deterministic regeneration, and 18 focused catalog/matrix/harness tests. A differential adapter protocol now requires exit/output/error/payload/state/event/persistence/selector/multiwindow observations and only documented JSON-pointer normalization. Full release gates and push remain before this baseline batch is complete.
- Next family after baseline push: independently audit and verify the 12-entry foundational workspace lifecycle batch: v2 `workspace.close/create/current/list/rename/select` plus CLI `close-workspace/new-workspace/current-workspace/list-workspaces/rename-workspace/select-workspace`. All are priority 0 and currently `implemented_unverified`; the six CLI dependency edges remain inside the batch. Use isolated backend/client worktrees and do not promote any entry without live/differential evidence.
