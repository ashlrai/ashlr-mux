...
- Next after push: audit and implement canonical `join-pane` / `pane.join`, including required target pane, optional source pane/surface/workspace/window selectors, focus/no-focus precedence, shared surface-move behavior, response, and exact errors. The overall parity goal is not complete.
