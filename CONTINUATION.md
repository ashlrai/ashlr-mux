...
- Canonical `drag-surface-to-split` / `surface.drag_to_split` is fully implemented and verified, ready to push. Its distinct command name, error prefixes, concrete help, v2 method, canonical output, capability advertisement, and real named-pipe executable route all reuse the exact verified `split-off` parser/formatter/backend mutation without duplicating layout logic. The red mapping/help/output/executable/catalog contracts, shared-branch simplification, two focused test cycles, all 275 core tests, all 87 config tests, all 196 CLI library + 21 binary tests and executable regressions, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Next after push: audit and implement canonical `swap-pane` / `pane.swap`, pinning pane selector, workspace/window context, focus, layout-tree exchange, response, and exact errors. The overall parity goal is not complete.
