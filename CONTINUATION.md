...
- Core `right-sidebar` remote-control parity is implemented and ready to push: the CLI now matches canonical toggle/show/hide/focus/set/mode and shorthand validation, normalizes `vault` to raw `sessions`, supports workspace/window/ambient targeting plus `--no-focus`, prints JSON only for mode queries, and has concrete help. A managed native state stays synchronized with local React actions; socket changes emit a native event that drives visibility/mode/focus; Feed and Dock remain explicitly unavailable until their real beta panels land. Focused red/green tests, executable help/error probes, the complete `cmux-cli` suite (101 library + 13 binary tests), all 1,324 desktop-web tests, web typecheck/build, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
