...
- Next after push: implement the distinct tmux-compat `resize-pane` / `resizep` orchestration (`-t`, absolute `-x`/`-y` via pane cell metrics, directional default amount 5, percent stripping, and OMX-HUD height exception), then resume the broader missing-command audit. The overall parity goal is not complete.
