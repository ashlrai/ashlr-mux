...
- Canonical `reorder-surface` / `surface.reorder` is fully implemented and verified, ready to push: positional or `--surface` targets; numeric/ref/id normalization; workspace/window context; `--index`, before/after aliases, case-insensitive canonical focus booleans with `false` default; exact target/same-pane validation; canonical response/plain output; and concrete help. The pure core mutation is pinned to the exact bonsplit insertion-offset contract: destination clamp to `0...count`, forward-move subtraction after removal, self/self+1 no-op, pinned/unpinned tier clamping, preserved selection for `focus=false`, and moved-surface selection for `focus=true`. The desktop resolves direct IDs across workspaces or scoped refs, rejects cross-pane anchors, mutates under the session lock, and emits stable window/workspace/pane/surface handles. The red route/output/help tests, executable core bonsplit/pin/focus test, real built-executable named-pipe regression, two focused cycles, all 266 core tests, all 87 config tests, all 192 CLI library + 18 binary + 14 executable-regression tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Next after push: audit and implement canonical cross-pane `move-surface` / `surface.move`, now the adjacent advertised layout command without a Windows CLI/backend route. Pin destination-pane, before/after/index, empty-pane collapse, pin-tier, focus, and response semantics before writing the red contract. The overall parity goal is not complete.
