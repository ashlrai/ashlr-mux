...
- Nested JSON hook installers are implemented and ready to push for Gemini, Grok, Copilot, CodeBuddy, Factory, and Qoder: direct and setup aliases resolve canonical home/environment paths, preserve unrelated config/user hook groups, replace only cmux-owned groups idempotently, install lifecycle plus 120-second Feed bridges, and normalize Grok's schema to 5/120 seconds while other agents retain millisecond units. Red parser/shape/unit tests, shared definition/path/confirmation/ownership simplifications, isolated executable install/reinstall smokes for all six agents, all 127 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
