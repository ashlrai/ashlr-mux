...
- Windows WebSocket PTY parity slice is implemented and ready to push: the canonical authentication, lease, WebSocket/RPC, persistent-session, replay, resize, backpressure, input-sequence acknowledgement, and shutdown implementation is now shared across platforms; small Unix and Windows launch/resize adapters supply `creack/pty` and native ConPTY respectively. Windows defaults to PowerShell, supports `cmd.exe` and Unix-like explicit shells, and writes oversized startup commands to platform-appropriate temporary scripts. The previous Windows `not implemented in M0` runtime stub is gone.
