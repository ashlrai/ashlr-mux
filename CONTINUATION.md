...
- The Windows desktop test loader is fixed and locally certified at `f4642e639`. Exact PE analysis showed the unit harness imported `comctl32.dll!TaskDialogIndirect` without a Common-Controls v6 activation manifest, binding base comctl32 v5.82 and exiting before the harness with `0xc0000139`. A cfg(test)+Windows+MSVC-only resource library now supplies exactly one v6 identity without changing the normal Tauri binary manifest. Independent fresh-target verification passed under Windows PowerShell 5.1 and PowerShell Core, enumerated 448 tests, ran a targeted desktop test, and confirmed the normal desktop build. The fresh full release gate also passed. The now-runnable full desktop suite exposes seven deterministic runtime failures (440 pass/7 fail/1 ignored) across browser text/binary classification, two workspace-current assertions, remote proxy timeout, relay socket 10053, and session panel-proxy classification. Evidence is in `docs/parity/evidence/desktop_loader_runtime_2026-07-11.json`; those seven reds are the next runtime-certification work.
