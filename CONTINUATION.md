...
- Canonical no-socket Config reference modes are ready to push: `config help`, `config path|paths`, and `config docs|documentation` now execute locally, with exact canonical usage/errors and shared Settings/Docs output so the aliases cannot drift. Config doctor and font-size reads remain explicit subsequent slices. The red local-dispatch test, shared-output unit test, help/path/docs executable smoke, explicit unsupported-mode smoke, reuse-based simplification, all 155 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
