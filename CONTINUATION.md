...
- Canonical tmux-compatible `resize-pane` / `resizep` orchestration is implemented and verified for direct/ref/index/title, caller-environment, globally searched pane, and focused-pane targets, ready to push. `__tmux-compat` now has a dedicated multi-call executor and canonical clustered/attached `-t/-x/-y` plus `-D/-L/-R/-U` parsing. Absolute width/height strips `%`, obtains live pane cell metrics through `pane.list`, converts cells to pixels, and calls `pane.resize`; directional mode uses L→U→D→R precedence, x-over-y amount, invalid/default 5, and minimum 1. Height-only resize detects current and legacy OMX HUDs through surface metadata/text and remains a quiet no-op. Successful resize and no-op branches preserve tmux's empty stdout. Workspace refs are resolved before bare zero-based tmux indexes, stable numeric pane hashes are accepted, and global pane search matches canonical caller-first behavior. The tuple-heavy first implementation was simplified into a named resolved-target object after strict Clippy. Two focused cycles and a real three-request named-pipe executable regression pass, along with all 287 core tests, all 87 config tests, all 207 CLI library tests plus binary/executable suites, golden tests, strict Clippy, desktop all-target check/test-link/build, all 1,337 web tests, typecheck/build, formatting, and whitespace checks. Remaining tmux previous-workspace selectors (`!`, `^`, `-`) depend on the separately missing canonical `workspace.last` state/API and are the immediate next slice; `__tmux-compat -V` and other shim commands remain in the broader audit.
...
- Next after push: implement canonical `workspace.last` state/control behavior, wire tmux `!`/`^`/`-` target resolution to it, then close `__tmux-compat -V` and resume the broader missing-command audit. The overall parity goal is not complete.
