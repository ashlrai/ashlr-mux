...
- Canonical hidden `diff-viewer-server` is fully implemented and verified, ready to push: `--root` parsing and exact errors; canonicalized directory validation; loopback ephemeral binding; `.server.json` identity/protocol state; port handoff; concurrent GET/HEAD serving; health, manifest-file, deferred-wait, refs, and branch-regeneration routes; canonical no-store/isolation/nosniff/origin headers; strict token/request-path/MIME/extension/duplicate/root-jail manifest validation and refresh; local 64 KiB file streaming; exact trusted GitHub pull-diff allowlisting and cancellable `curl` streaming; remote `HEAD` without a fetch; persisted branch-session token/repo/group authorization; and bounded replacement waits with the canonical failure page. Branch regeneration now targets the server's injected trusted root and writes token-bound manifests. A real `groupID` casing regression was caught by the executable refs/branch test and fixed. The red dispatch contract, parser/URL unit tests, six built-executable loopback/security/refresh/wait/refs/branch tests, accept-loop and streaming cleanup simplification, repeated focused retests, all 263 core tests, all 87 shared-config tests, all 181 CLI library + 16 binary + 6 diff-server + 2 SIGPIPE executable-regression tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Suggested next slice after pushing: resume the canonical-vs-Windows CLI inventory at the next concrete unported/no-op command, confirm its complete contract from canonical source and tests, then repeat red test -> implementation -> simplification -> full gates -> push.
