...
- Generated agent lifecycle hook routing is implemented and ready to push: `cmux hooks AGENT session-start|prompt-submit|stop|notification|agent-response|approval-response|shell-*|session-end|session-finalize` now transforms into the existing bounded Feed bridge rather than incorrectly re-entering the installer. Kiro's installed lifecycle commands therefore ingest/no-op safely and never mutate config at runtime. The red dispatcher test, direct transformation simplification, executable outside-surface/no-mutation smoke, all 125 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
