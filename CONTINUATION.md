...
Urgent compaction checkpoint, 2026-07-11:

- Hosted Windows run `29170180137`, job `86590035874`, passed real sidecars, bootstrap tests, strict Clippy, workspace check, type drift, 24 bridge tests, and the full native suite at exactly 453 passed/0 failed/1 ignored. It then exposed two stale Python source-contract assertions: the failure envelope omitted load-bearing `error.data`, and the local Tauri command list/count omitted `active_callback_scheme`. The first correction review correctly failed a parser blind spot for async/visibility/attributes and substring handler matches. Correction `8a9b507a5` now behaviorally pins structured `NativeBridgeError.data`, scopes additive NativeReply fields, discovers attributed/visible/async/unsafe local commands, and checks exact handler entries with adversarial collision coverage. Independent verification passed exact source commit `7a074b911`; the integrated release gate at `8a9b507a5` passed the full desktop matrix (453/0/1, contracts, integration, E2E skipped=0), all release suites, and parity checks. Hosted rerun `29171597399`, job `86593651884`, then passed the exact pushed checkpoint `9685ad43c`: 25 bridge tests, 453/0/1 native, contracts, integration, E2E with zero skips, and the complete desktop matrix. Windows runtime certification is green; only the frozen canonical differential blocks matrix promotion.
- The seven deterministic hosted Windows desktop reds are fixed at `ca402eb592`. Three opaque CONNECT-prefix observations now classify as binary through one tunnel helper; two `workspace.current` tests now encode canonical manager-routing/stable-identity semantics without production changes; and the remote transport now drains upstream before terminal shutdown, preserves daemon write half-close, uses canonical 8s/4s write/close deadlines, correlates concurrent responses/events, and closes/unregisters exactly once on explicit or final ownership release. A first independent review failed final-drop ownership and deterministic barrier coverage; the red/green correction pair `edae23b11`/`ca402eb59` closed both. The final independent verifier passed the full native suite plus barrier/original relay 100x, final-drop/Both-drop/out-of-order 50x stress. The fresh uninterrupted release gate passed exactly 453 desktop tests with 0 failures and 1 ignored (454 enumerated), dual-shell manifest verification, normal desktop build, strict Rust 1.97 Clippy, all core/config/CLI/runtime suites, all 1,337 web/webview tests, typecheck/build, contracts, frozen extractors, matrix validators, parity tests, formatting, and diff checks. Evidence is in `docs/parity/evidence/desktop_runtime_seven_2026-07-11.json`. No matrix row is promoted before hosted Windows green plus a frozen canonical differential.
- The Windows desktop test loader is fixed and certified at `f4642e639`. Exact PE analysis showed the unit harness imported `comctl32.dll!TaskDialogIndirect` without a Common-Controls v6 activation manifest, binding base comctl32 v5.82 and exiting before the harness with `0xc0000139`. A cfg(test)+Windows+MSVC-only resource library now supplies exactly one v6 identity without changing the normal Tauri binary manifest. Independent fresh-target verification passed under Windows PowerShell 5.1 and PowerShell Core, enumerated 448 tests, ran a targeted desktop test, and confirmed the normal desktop build; the fresh full release gate also passed. Hosted Windows CI run `29168099312` then entered the harness and reported exactly 440 pass/7 fail/1 ignored in 10.15 seconds with no loader error. The seven deterministic reds cover browser/session proxy text-vs-binary classification, two workspace-current assertions, remote proxy timeout, and relay socket 10053. Evidence is in `docs/parity/evidence/desktop_loader_runtime_2026-07-11.json`; those seven reds are the next runtime-certification work.
- The 12-entry workspace lifecycle batch is implemented and locally certified through three independent reviews. The first two reviews correctly failed payload formatting, ID modes, selector normalization, stale selection, routing, create layout/group/environment semantics, background-window events, focus, close history, teardown, and remote rename; five separate red/green correction pairs closed those defects. The third verifier PASS at `1b92e8f49` found real production consumers for stable refs, stale selected identity, public close/reopen restoration, dependent-state teardown, SSH tmux rename dispatch/ack, and same-title events. The uninterrupted release gate at `978b238ee` passed 290 core tests, 219 CLI library tests plus all executable/integration groups, golden/config suites, desktop check/link/build, strict core/config/CLI Clippy, all 1,337 web/webview tests, typecheck/build, generated-type and desktop-contract checks, all parity validators/tests, formatting, and diff checks. Hosted Windows CI run `29166101367` at `fb0d52023` then passed frozen dependencies, contracts/web build, sidecar staging, bootstrap tests, Rust 1.97 strict Clippy, full workspace check, type drift, and 24 bridge tests before the native desktop test executable failed before the harness with `0xc0000139 STATUS_ENTRYPOINT_NOT_FOUND`. All 12 rows deliberately remain `implemented_unverified`; exact evidence is recorded in `docs/parity/evidence/workspace_lifecycle_2026-07-11.json`.
- The parity program now uses a frozen, generated, independently verified matrix baseline. Canonical is pinned to `e1825d40d52b4ae4f4bcb0b7e0dfc744dd20a452` (2026-07-09) and Windows evidence to `689854602b58dd1f6ee24f3d6a94182cfa8c7423`; upstream movement no longer silently changes the target. Deterministic extractors catalog 174 accepted CLI tokens (158 public + 16 hidden), 293 v2 methods (251 release + 42 debug), 19 product/runtime requirements, and exact Windows route/help/unsupported evidence. The joined matrix has 486 unique entries: 233 `implemented_unverified`, 253 `missing`, zero resolved, strict parity `0/486 (0.0%)`, completion `0/486 (0.0%)`, and secondary core-local workflow `0/277 (0.0%)`. Source presence is capped at `implemented_unverified`; 12 explicit `not_supported` routes remain missing. The first independent audit failed five invalid product paths, 21 dangling alias dependencies, and six undercounted conditional/helper CLI routes; all were fixed, dependency/source validation was made exhaustive, and the fresh verifier PASS at `446746eaf` confirms 207 pinned blobs, no dangling IDs, schema/count/status arithmetic, deterministic regeneration, and 18 focused catalog/matrix/harness tests. A differential adapter protocol now requires exit/output/error/payload/state/event/persistence/selector/multiwindow observations and only documented JSON-pointer normalization. The verified baseline was pushed at `eb77096f4`.
- Next batch: push the exact runtime-seven evidence commit, dispatch hosted CI, and require the Windows desktop job to pass all native tests. Record that immutable run without promoting matrix rows, then derive the next 5-15-entry dependency-coherent family from the frozen matrix and take it through contract, red/green/simplify/green, independent verification, full gate, evidence, and verified push.
- Canonical tmux-compatible `select-window` / `selectw` is fully implemented and verified, ready to push. The shim accepts attached or separate `-t`, resolves canonical workspace ID/ref/index/title, caller-environment, focused, and previous-workspace selectors through the existing tmux resolver, calls `workspace.select` with the stable ID, and preserves quiet stdout. The implementation adds no duplicate parsing or resolution path. Two focused passes plus a real two-request named-pipe executable regression pass, along with all 288 core tests, all 87 config tests, all 215 CLI library + 29 binary tests and 19 executable regressions, golden tests, strict Clippy, desktop all-target check/test-link/build, all 1,337 web tests, typecheck/build, formatting, and whitespace checks.
- Next after push: audit canonical tmux `kill-window` / `killw` and `kill-pane` / `killp`, then take the smallest verified mutation slice. The overall parity goal is not complete.
- Canonical tmux-compatible `select-pane` / `selectp` is fully implemented and verified, ready to push. The shim accepts attached or separate `-t` targets, reuses the verified caller/workspace/global tmux pane resolver, calls `pane.focus` with stable IDs, and preserves quiet stdout. Canonical `-P` style and `-T` title compatibility requests are quiet no-ops that return before any socket lookup. The resize-only parser was simplified into one command-configured attached/clustered flag parser, and the existing absolute-resize regression reverified it. Two focused passes plus a real three-request named-pipe executable regression pass, along with all 288 core tests, all 87 config tests, all 214 CLI library + 29 binary tests and 18 executable regressions, golden tests, strict Clippy, desktop all-target check/test-link/build, all 1,337 web tests, typecheck/build, formatting, and whitespace checks.
- Canonical `focus-pane` / `pane.focus` is fully implemented and verified, ready to push. The CLI now keeps `focus-pane` distinct from `focus-panel`, accepts positional or `--pane` ID/ref/index plus workspace/window and ambient scope, emits the exact missing-pane error, exposes concrete pane-specific help, calls `pane.focus`, and prints `OK pane:N`; a real named-pipe executable regression verifies normalized request and output. The desktop advertises/routes the method, preserves canonical TabManager/workspace/pane error ordering and payloads, resolves pane refs/IDs only inside the selected or explicit workspace/window, focuses the owning native window, and returns canonical window/workspace/pane handles. The atomic state mutation selects the workspace and focuses the pane's already-selected surface without changing its tab selection. Simplification replaced a session-local layout walker with the pure core `focus_pane_target` primitive. Two focused passes, all 288 core tests, all 87 config tests, all 212 CLI library + 29 binary tests and 17 executable regressions, golden tests, strict Clippy, desktop all-target check/test-link/build, all 1,337 web tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `pane.surfaces` and the three distinct pane-list CLI routes are fully implemented and verified, ready to push. The desktop advertises and handles `pane.surfaces`, resolves pane IDs globally within a requested window or pane refs/focus within selected/explicit workspace scope, returns canonical window/workspace/pane handles plus pane-local ordered surface rows, and preserves canonical unavailable/not-found errors. `list-panes`, `list-pane-surfaces`, and `list-panels` now call `pane.list`, `pane.surfaces`, and `surface.list` respectively, inherit ambient workspace scope, expose distinct concrete help, and print exact canonical empty/row/focus/selection text. A real named-pipe executable regression verifies request normalization and output. The first verification pass caught an unnecessary boxed iterator compile failure; simplification replaced it with direct scoped/global branches. The broader CLI pass then caught and fixed missing ambient scope registration for the two pane methods. This slice also fixes the earlier `workspace.last` capability placement by registering it in the main control-method catalog. Two focused passes, all 287 core tests, all 87 config tests, all 212 CLI library + 28 binary tests and 16 executable regressions, golden tests, strict Clippy, desktop all-target check/test-link/build, all 1,337 web tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `__tmux-compat -V` / `-v` is fully implemented and verified, ready to push. The tmux executor now returns an explicit quiet/output result: both version spellings print exactly `tmux 3.4`, report no mutation, and never resolve or connect to the control socket; resize and no-op commands retain empty stdout. Pure tests reject any attempted socket callback, and a real executable regression succeeds against a deliberately nonexistent named pipe. The result-object refactor replaced an output side channel and the second focused pass reverified absolute resize. Both focused cycles, all 287 core tests, all 87 config tests, all 210 CLI library + 27 binary tests and 15 executable regressions, golden tests, strict Clippy, desktop all-target check/test-link/build, all 1,337 web tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `last-window` / `workspace.last` is fully implemented and verified, ready to push. Per-window runtime focus history is seeded on startup/restore, centrally records every notified workspace selection, caps at 50 entries, truncates forward branches after ordinary selection, and navigates backward without re-recording. It skips deleted/stale workspaces exactly like canonical and returns `not_found: No previous workspace in history` when no distinct valid entry remains. The control route resolves explicit windows or the active registered window, focuses it, and returns canonical workspace/window handles. Top-level `last-window` has concrete help, normalized optional window scope, `OK workspace:N`, and a real named-pipe executable regression. Tmux resize targets `!`, `^`, and `-` now delegate through `workspace.last` before resolving the pane. The initial index-zero default was replaced during simplification with active-window routing. Two focused cycles, all 287 core tests, all 87 config tests, all 209 CLI library + 27 binary tests and executable suites, golden tests, strict Clippy, desktop all-target check/test-link/build, all 1,337 web tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical tmux-compatible `resize-pane` / `resizep` orchestration is implemented and verified for direct/ref/index/title, caller-environment, globally searched pane, focused-pane, and previous-workspace (`!`/`^`/`-`) targets, ready to push. `__tmux-compat` now has a dedicated multi-call executor and canonical clustered/attached `-t/-x/-y` plus `-D/-L/-R/-U` parsing. Absolute width/height strips `%`, obtains live pane cell metrics through `pane.list`, converts cells to pixels, and calls `pane.resize`; directional mode uses L→U→D→R precedence, x-over-y amount, invalid/default 5, and minimum 1. Height-only resize detects current and legacy OMX HUDs through surface metadata/text and remains a quiet no-op. Successful resize and no-op branches preserve tmux's empty stdout. Workspace refs are resolved before bare zero-based tmux indexes, stable numeric pane hashes are accepted, and global pane search matches canonical caller-first behavior. The tuple-heavy first implementation was simplified into a named resolved-target object after strict Clippy. Two focused cycles and a real three-request named-pipe executable regression pass, along with all 287 core tests, all 87 config tests, all 209 CLI library tests plus binary/executable suites, golden tests, strict Clippy, desktop all-target check/test-link/build, all 1,337 web tests, typecheck/build, formatting, and whitespace checks. `__tmux-compat -V` and other shim commands remain in the broader audit.
- Canonical `pane.list` is fully implemented and verified, ready to push. The desktop advertises the method; resolves registered-window and selected/explicit-workspace routing with canonical unavailable/not-found errors; walks nested splits in leaf order; returns stable pane IDs/refs, workspace-global surface refs, selected/focused state, pixel frames, container dimensions, and live terminal columns/rows/cell-pixel estimates. A shared terminal grid-size query avoids duplicating PTY state. The first review caught and fixed pane-local surface-ref numbering; the simplification pass also made invalid restored divider values project at canonical 50% and removed impossible null surface-ref entries. Both focused desktop compile cycles, all 287 core tests, all 87 config tests, all CLI and golden suites, desktop all-target checking/test linking/build, strict Clippy, all 1,337 web tests, web typecheck/build, formatting, and whitespace checks pass. This is the canonical prerequisite for tmux absolute pane resizing.
- Canonical `resize-pane` / `pane.resize` is fully implemented and verified, ready to push. The modern CLI preserves optional pane/workspace/window selectors, exact L/R/U/D precedence and default, canonical amount fallback/error behavior, concrete help, `OK pane:N`, and a real named-pipe route. The desktop advertises and routes relative or absolute-pixel requests, resolves persisted focused/explicit panes within registered-window and selected/explicit-workspace context, uses physical window dimensions, returns canonical handles/divider fields and exact validation/state errors, and notifies the session. The transactional pure core walks nearest split ancestors in canonical postorder, chooses only borders adjacent in the requested direction, converts pixels against each actual nested split span, clamps dividers to 0.1...0.9, and leaves every failure byte-identical. Session splits now carry backward-compatible stable UUID identities, with generated TypeScript bindings drift-clean. Selection was extracted into one compiled resolver before the second focused pass. Both focused cycles, all 287 core tests, all 87 config tests, all CLI suites including the real executable pipe, desktop all-target checking/test linking/build, strict Clippy, all 1,337 web tests, core-types/web typechecks and web build, formatting, and whitespace checks pass.

- Canonical `last-pane` / `pane.last` is fully implemented and verified, ready to push. The CLI normalizes optional workspace/window scope, prints canonical `OK pane:N`, exposes concrete help, and routes through the real named pipe. The desktop advertises the method, resolves registered-window and selected/explicit workspace context, returns exact unavailable/workspace/focused/alternate errors and canonical pane/surface/window/workspace handles, and focuses the native owner window. The pure core validates the current pane then selects the first different pane in layout order exactly like canonical. This slice also closes the previously documented focus-persistence gap: `focused_panel_id` is backward-compatible session state, generated core-types are drift-clean, new/selected workspaces initialize/update it, web pointer and keyboard focus persist it through a no-selection-change command, restored snapshots resynchronize the low-latency focus store, and repeated `pane.last` calls truly alternate. The no-allocation tree walk replaced the first implementation before the second focused pass. All 283 core tests, all 87 config tests, all CLI library/binary/executable suites, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, core-types drift/typecheck, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `join-pane` / `pane.join` was pushed as `c8c30611c`. The CLI requires and normalizes the target pane; preserves optional source pane/surface/workspace/window selectors, `focus=false`, `--no-focus`, exact conflict/missing-target errors, `OK` output, concrete help, and a real named-pipe route. `break-pane` and `join-pane` share one selector/focus parser. The desktop advertises the method, resolves explicit surfaces or the selected surface of a direct/scoped source pane with canonical missing/unresolved errors, translates target refs/IDs, and delegates the mutation plus response/errors to the already-verified `surface.move` path. Source resolution was extracted into a pure compiled desktop contract before the second focused pass. All 281 core tests, all 87 config tests, all CLI library/binary/executable suites, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `break-pane` / `pane.break` was pushed as `d89d45540`. The CLI preserves optional pane/surface/workspace/window selectors, canonical source precedence, `focus=false`, `--no-focus`, exact conflicting-focus errors, `OK` output, concrete help, and a real named-pipe route. The desktop resolves registered-window ownership plus selected/explicit workspace context, derives the selected surface from an explicit pane or the focused fallback, advertises the method, creates and identifies the new workspace/pane, returns canonical handles/errors, and optionally focuses the result. The transactional core permits even a singleton source, transfers panel metadata and pane-local state, leaves invalid requests byte-identical, preserves selection when unfocused, and mints a new pane identity in the state layer. The older move-to-new-workspace path now reuses this primitive, removing duplicate detach/metadata/placement logic while retaining its singleton rejection. Two focused post-simplification cycles, all 281 core tests, all 87 config tests, all CLI library/binary/executable suites, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `swap-pane` / `pane.swap` was pushed as `aad2f9815`. The CLI requires and normalizes both pane selectors, preserves optional workspace/window context and canonical `focus=false`, prints `OK`, exposes concrete help, and routes through the real named pipe. The desktop resolves direct pane IDs globally across registered windows, constrains the target to the source workspace, resolves refs within explicit or selected scope, advertises the method, returns canonical pane/surface handles, and optionally selects/focuses the owning workspace/window. The transactional core swaps only the selected surfaces while preserving both pane identities; matches canonical two-move ordering and pin tiers; preserves singleton panes through equivalent placeholder semantics; and rejects same, missing, or surface-less panes atomically. Duplicate pane traversal was removed before the second focused cycle. All 278 core tests, all 87 config tests, all CLI library/binary/executable suites, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `drag-surface-to-split` / `surface.drag_to_split` was pushed as `9b6d6054a`. Its distinct command name, error prefixes, concrete help, v2 method, canonical output, capability advertisement, and real named-pipe executable route all reuse the exact verified `split-off` parser/formatter/backend mutation without duplicating layout logic. The red mapping/help/output/executable/catalog contracts, shared-branch simplification, two focused test cycles, all 275 core tests, all 87 config tests, all 196 CLI library + 21 binary tests and executable regressions, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Next after push: derive the next slice from a canonical-versus-Windows control-method catalog diff, then implement the highest-leverage missing method and its CLI consumers. The overall parity goal is not complete.

- Canonical `split-off` / `surface.split_off` was pushed as `92c062571`: required `--surface`/`--panel`, full and one-letter directions, workspace/window source-context validation, canonical default `focus=false`, errors, response/plain output, concrete help, and a real named-pipe executable route. The pure mutation rejects a single-tab source atomically, preserves the original pane and its selection, clones pane-local presentation state for the moved surface, inserts the new pane on the correct side/orientation, and lets the stateful layer mint its identity. The desktop resolves direct IDs globally and refs within scoped/selected workspaces across registered windows, distinguishes source/window/workspace/cardinality failures, optionally focuses the owning window/workspace, and advertises the method. The red layout/atomicity/CLI/output/help tests, compiled backend multiwindow resolver and direction-alias tests, two focused post-simplification cycles, all 275 core tests, all 87 config tests, all 195 CLI library + 21 binary tests and executable regressions, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.

- Canonical `move-workspace-to-window` / `workspace.move_to_window` was pushed as `de7bc73ce`: required normalized workspace/window selectors, canonical errors, response/plain output, concrete help, and a real named-pipe executable route. The transactional core move resolves the source before the destination, detaches group membership and dissolves moved anchors, preserves source/destination selection by identity, bootstraps an emptied source window, normalizes pinned insertion, supports canonical same-window detach/attach behavior, and leaves invalid requests byte-identical. Tauri windows now register/unregister authoritative tab managers, receive per-window snapshot/event projections and titles, expose their real workspace counts/selections, and can receive the moved workspace with optional focus. The red ownership/atomicity/same-window/CLI/output/help tests, compiled backend global-id/local-ref and per-window projection tests, two focused post-simplification cycles, all 273 core tests, all 87 config tests, all 194 CLI library + 20 binary tests and executable regressions, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.

- Canonical `move-surface` / `surface.move` was pushed as `c21f7af93`: positional or `--surface` source selectors; numeric/ref/id normalization; pane/workspace/window destinations; anchor-over-pane-over-workspace-over-window precedence; before/after aliases; optional insertion index and canonical focus booleans; canonical response/plain output; and concrete help. The transactional pure core mutation covers same-pane reorder, cross-pane split collapse, cross-workspace transfer of every panel-scoped metadata record, pinned/unpinned insertion tiers, focus/selection policy, and byte-identical rejection of invalid destinations. Shared detach/attach bookkeeping now also simplifies `move_panel_to_new_workspace`. The red core/CLI/output/help tests, compiled desktop resolver-precedence test, real built-executable named-pipe regression, two focused post-simplification cycles, all 269 core tests, all 87 config tests, all 193 CLI library + 19 binary tests and executable regressions, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.

- Canonical `reorder-surface` / `surface.reorder` was pushed as `e44a9dc07`: positional or `--surface` targets; numeric/ref/id normalization; workspace/window context; `--index`, before/after aliases, case-insensitive canonical focus booleans with `false` default; exact target/same-pane validation; canonical response/plain output; and concrete help. The pure core mutation is pinned to the exact bonsplit insertion-offset contract: destination clamp to `0...count`, forward-move subtraction after removal, self/self+1 no-op, pinned/unpinned tier clamping, preserved selection for `focus=false`, and moved-surface selection for `focus=true`. The desktop resolves direct IDs across workspaces or scoped refs, rejects cross-pane anchors, mutates under the session lock, and emits stable window/workspace/pane/surface handles. The red route/output/help tests, executable core bonsplit/pin/focus test, real built-executable named-pipe regression, two focused cycles, all 266 core tests, all 87 config tests, all 192 CLI library + 18 binary + 14 executable-regression tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.

- Canonical atomic `reorder-workspaces` / `workspace.reorder_many` was pushed as `a05c6f0e2`: exact `--order` presence/empty-item errors; comma-separated numeric/ref/id normalization; optional window scope; `--dry-run`; canonical applied/plan text; and concrete help. The desktop advertises the method, preserves request order while resolving refs/UUIDs, validates duplicates before unknown workspaces, returns canonical error and plan/events payloads, and applies the shared golden-pinned batch planner atomically under one session lock. The core application rebuilds requested leading order inside pinned/unpinned tiers, keeps unmentioned peers stable, restores group contiguity/anchor order, follows selection, and leaves state unchanged for dry runs or validation errors. Dry-run response refs come from a fully planned clone without mutating authoritative state. The red route/output/help tests, two executable core mutation/group/atomicity tests, compiled desktop parser test, real built-executable named-pipe batch regression, two simplification/retest cycles, all 265 core tests, all 87 config tests, all 191 CLI library + 17 binary + 13 executable-regression tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.

- Canonical top-level `reorder-workspace` was pushed as `2a6ff8cb9`: positional or `--workspace` source selectors; `--index`, `--before`/`--before-workspace`, and `--after`/`--after-workspace` targets; numeric/ref/id normalization; optional window scope; `--dry-run`; and concrete command help. The desktop validates window scope without falling through to the first window, plans dry runs by applying the exact shared core reorder algorithm to a clone, emits the canonical single-reorder payload/plan/events shape, and mutates only non-dry requests. Plain CLI output matches canonical applied/plan summaries. The red route, output, and help tests, compiled desktop target/window contract tests, real built-executable named-pipe request/output regression, simplification pass, repeated focused tests, all 263 core tests, all 87 config tests, all 190 CLI library + 17 binary + 12 executable-regression tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.

- Canonical no-socket `vm-pty-connect` was pushed as `3902a6d2c`: last-value `--config`/`--id` parsing and exact usage/unknown-long-flag errors; tilde expansion; read-then-best-effort-delete one-time camelCase JSON configs even before decode; `ws`/`wss` with native-root Rustls and custom headers; 15-second open timeout; auth JSON with token/session/attachment/current size; ready-frame gate; cross-platform raw-mode restoration; binary stdin/stdout; Windows-friendly 250 ms resize polling; five-second ping/pong liveness; graceful close/BrokenPipe handling; and concrete help. The red dispatch test, three parser/config-consumption tests, real built-executable local-WebSocket auth/binary-I/O/config-deletion regression, Windows console-size correction, two focused test cycles, all 263 core tests, all 87 config tests, all 189 CLI library + 16 binary + 11 executable-regression tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass. With this mapping, every `PreSocketAction` has a concrete executor, so the obsolete generic no-socket failure branch and 20 redundant dispatch returns were removed.

- Canonical no-socket `remote-daemon-status` was pushed as `1bb9610ac`: it preserves first-value `--os`/`--arch` and `--name=value` parsing, target normalization, canonical `~/.local/state/cmux/remote-daemons/<version>/<goos>-<goarch>/cmuxd-remote` lookup, streaming SHA-256 verification, manifest/entry/cache/release/checksum/attestation fields, nightly vs release signer workflows, sorted pretty JSON, exact text ordering, missing-manifest diagnostics, dev fallback reporting, and concrete help. Windows discovers valid manifests from an explicit JSON/path seam, packaged executable/resource sidecars, or ancestor release-asset directories, skipping malformed candidates; defaults to its real Go host tuple. Explicit acronym wire names (`releaseURL`, `checksumsURL`, `downloadURL`) fixed a real manifest-decoding regression. The red dispatch test, three renderer/parser/cache tests, two built-executable manifest/missing-manifest tests, candidate/hash simplification and focused retest, all 263 core tests, all 87 shared-config tests, all 185 CLI library + 16 binary + 6 diff-server + 2 daemon-status + 2 SIGPIPE executable-regression tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.

- Canonical hidden `diff-viewer-server` was pushed as `d83c66f88`: `--root` parsing and exact errors; canonicalized directory validation; loopback ephemeral binding; `.server.json` identity/protocol state; port handoff; concurrent GET/HEAD serving; health, manifest-file, deferred-wait, refs, and branch-regeneration routes; canonical no-store/isolation/nosniff/origin headers; strict token/request-path/MIME/extension/duplicate/root-jail manifest validation and refresh; local 64 KiB file streaming; exact trusted GitHub pull-diff allowlisting and cancellable `curl` streaming; remote `HEAD` without a fetch; persisted branch-session token/repo/group authorization; and bounded replacement waits with the canonical failure page. Branch regeneration targets the server's injected trusted root and writes token-bound manifests. A real `groupID` casing regression was caught by the executable refs/branch test and fixed. The red dispatch contract, parser/URL unit tests, six built-executable loopback/security/refresh/wait/refs/branch tests, accept-loop and streaming cleanup simplification, repeated focused retests, all 263 core tests, all 87 shared-config tests, all 181 CLI library + 16 binary + 6 diff-server + 2 SIGPIPE executable-regression tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.

- Canonical hidden SIGPIPE diagnostics were pushed as `ac8ce1e86`: `__sigpipe-inspect` supports stdout or `--out`; `__sigpipe-probe` supports default/spawn, spawn-stderr, and an observable Windows exec equivalent; and `__sigpipe-stdin-pipe-probe` writes the canonical 1 MiB payload to a child that closes stdin, ignores only BrokenPipe, enforces the five-second timeout, and returns `ok`. Windows reports the equivalent safe disposition (`signal=default`, no-sigpipe fields zero) because the platform has no SIGPIPE/fcntl flags. All top-level CLI stdout/stderr macros now use one non-panicking writer, so closed output pipes preserve command exit codes. The red local dispatch test, inspect/error unit tests, generic broken-writer test, two built-executable regression tests covering all modes and closed stdout/stderr, temp-cleanup/child-command simplification, focused retest, all 263 core tests, all 87 shared-config tests, all 178 CLI library + 16 binary + 2 executable-regression tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical no-socket `sessions` / `session-debug` was pushed as `4a5e8e356`: `sessions`, `sessions list`, `sessions debug`, and the legacy alias read all 18 canonical `~/.cmuxterm/*-hook-sessions.json` registries without a desktop socket; support the full agent/session/workspace/surface/cwd/state-dir/Codex-home/limit/all/JSON contract; sort and filter canonical default-visible records; inspect Codex indexes and nested/archived transcripts; resolve Claude config roots, direct/nested transcripts, and single-sibling workflow containers; and render canonical text/structured fields. Fork diagnostics reuse the ported launch trust, environment, and sanitizer policies; never execute captured commands; report OpenCode trust gates; and query real Windows PID creation time, executable, and argv. The red local dispatch test, eight isolated store/transcript/diagnostic/PID tests, path-root simplification and two-pass regression retest, empty executable smoke, all 263 core tests, all 87 shared-config tests, all 175 CLI library + 15 binary tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass. A Windows drive-relative Claude-project regression briefly produced five uniquely named test artifacts inside the crate; each tree was inspected, confirmed to contain only the generated three-byte transcript fixture, safely removed, and the fixed tests now assert/recheck that no artifact escapes the injected temp root.
- Canonical bare-path opening was pushed as `5bf543a71`: no-socket `cmux <path>` resolves `~`/relative components, opens a directory or an existing file's parent, emits the canonical missing-path error, refuses to recurse into the CLI executable, discovers the packaged sibling desktop (with an explicit override seam), launches it with `--open-path <absolute-directory>`, and prints `OK`. Desktop startup and single-instance argv routing extract only explicit existing directories, create/select cwd-rooted workspaces through the shared session mutation, and reveal/focus the main window. The red dispatch contract, directory/file-parent/recursion planner test, compile-lifetime correction, desktop import correction, core extraction simplification, executable core argv test, focused retest, desktop link, strict focused Clippy, all 263 core tests, all 87 shared-config tests, all 167 CLI library + 15 binary tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `window.list` / `window.current` real-window introspection was pushed as `37e25efce`: list returns canonical `{windows:[...]}` rows for every ordered Tauri window (id/ref/index/key/visible/workspace count/selected workspace identity), while current resolves a global numeric/ref/id `--window`, otherwise the actually focused Tauri window, then the first window. The first window retains the live session workspace metadata; auxiliary windows report zero/null until their independent session ownership is ported. CLI plain output keys selection from canonical `key` with legacy fallback. The red canonical-key formatter test, global current-window selector guard, shared identity reuse, focused core/CLI retest, desktop link, all 262 core tests, all 87 shared-config tests, all 165 CLI library + 15 binary tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `window displays` / `window display` were pushed as `6e9c7cf96`: the CLI namespace maps list/`--list` and move commands, applies global numeric/ref/id `--window` targeting, preserves canonical usage and plain output, and no longer falls through as unported. The desktop advertises both v2 methods; returns ordered name/index/main/frame metadata with nullable platform display IDs; moves one target or all Tauri windows without focus intent; preserves/clamps size; returns canonical moved identities; distinguishes missing windows before missing displays; and reports available names. Window label ordering/ref resolution moved into executable `cmux-core` tests. The red CLI mapping test, canonical formatter test, backend link, window-not-found ordering correction, identity extraction simplification/retest, executable usage/socket/target routing smoke, all 262 core tests, all 87 shared-config tests, all 165 CLI library + 15 binary tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `window default-display` was pushed as `16c1488ce`: the no-socket CLI queries/sets/clears `app.devWindowDisplay` with canonical text/JSON and trimming/clear precedence; shared config storage tolerates JSONC, refuses corrupt/non-object input, writes sorted pretty JSON atomically through symlinks, and prunes an empty `app`; DEBUG startup and runtime-created Tauri windows resolve displays by case-insensitive exact name, substring, or zero-based index, clamp to the work area, center without focus intent, and treat placement errors best-effort. The three red dispatch/storage/desktop contracts, executable core monitor test, focused config/CLI tests, desktop link, production-expect removal, core extraction simplification, strict focused Clippy, read-only executable query/error smoke, verified restoration of the prior missing user-config state, stale unported-audit correction/retest, all 262 core tests, all 87 shared-config tests, all 164 CLI library + 14 binary tests, strict core/config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical Config font-size mutation was pushed as `82e68459e`: `config set <key> <points>` and direct `<key> <points>` use a dedicated mutation dispatch plan; validate finite numbers before I/O; clamp/format canonically; preserve unrelated lines while replacing every matching assignment or appending one; follow symlink targets; create missing directories/files; then best-effort call `config.reload` and report `reloaded` or saved/reload-failed text/JSON without losing the saved edit. The red dispatch/file-output contracts, shared read/write parser simplification, bad-key/non-finite pre-I/O guards, six Config tests, post-simplification dispatch retests, invalid-value executable routing smoke, isolated executable write/clamp/reload-failure smoke, all 86 shared-config tests, all 162 CLI library + 14 binary tests, strict config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical `config reload` socket alias was pushed as `fadd9f7c9`: it routes case-insensitively to the existing `config.reload` control method and rejects extra arguments with exact canonical usage. The red dispatch test, arity/set-isolation simplification guards, focused retest, executable missing-socket routing/arity smoke, all 86 shared-config tests, all 160 CLI library + 14 binary tests, strict config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical no-socket Config font-size reads were pushed as `94b586aee`: `config get <sidebar-font-size|surface-tab-bar-font-size>` and the direct one-argument aliases read the same Ghostty candidate as the Windows desktop, use the last exact assignment, ignore comments/BOM, clamp to canonical ranges, apply canonical defaults, format to two decimal places without trailing zeroes, and emit text or sorted JSON with path/configured metadata. Ghostty candidate resolution moved into `cmux-config`, replacing the desktop duplicate. The red last-value/default/clamp/alias test, all 86 shared-config tests, test-only import correction, known desktop runtime-loader handling via no-run, expect-free simplification, focused retest, executable get/alias/default smoke, all 159 CLI library + 14 binary tests, strict config/CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical no-socket Config doctor was pushed as `767eecbeb`: `config doctor|check|validate` supports repeated `--path`/`--path=`, Windows primary config and optional legacy targets, nearest project `.cmux/cmux.json`/`cmux.json` discovery, faithful JSONC preprocessing, sorted keys, missing/empty/directory/non-object/parse findings, text or sorted JSON reports, and report-before-nonzero-error behavior. The red deferred-failure contract test, edge-finding/options test, pure project-discovery test, lexical Windows path simplification, focused retest, default-target smoke, executable stdout/stderr/exit smoke, all 158 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical no-socket Config reference modes were pushed as `302df8d7c`: `config help`, `config path|paths`, and `config docs|documentation` execute locally, with exact canonical usage/errors and shared Settings/Docs output so the aliases cannot drift. Config doctor and font-size reads remained explicit subsequent slices. The red local-dispatch test, shared-output unit test, help/path/docs executable smoke, explicit unsupported-mode smoke, reuse-based simplification, all 155 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical no-socket Settings modes were pushed as `58bf9bb17`: `settings path|paths` renders the canonical config/legacy/Ghostty/docs/schema/backup/reload text or sorted JSON; `settings docs|documentation` exactly reuses `docs settings`; and concrete help exposes canonical subcommands/targets. Separator and pre-separator JSON/help semantics share one parser with Docs, while open/target modes remain socket-backed. The red local-dispatch test, path/JSON/docs unit test, executable path/JSON/docs/help smoke, shared parser simplification, combined Docs/Settings executable retest, all 153 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical no-socket `cmux welcome` was pushed as `8a5b1fccc`: Windows prints the complete canonical light-theme true-color ANSI logo, tagline, 12 shortcut rows, Docs/Discord/GitHub/Email links, and command hints with exact fixed visual columns and leading/trailing blank lines. The red local-dispatch test, renderer content/ANSI test, fixed-column red regression, shared row-appender simplification, rebuilt executable retest, all 151 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Canonical no-socket `cmux docs` was pushed as `038260517`: Windows renders all seven canonical topics, aliases, text indexes/topic references, sorted pretty JSON, settings-only paths/backup/reload/Ghostty metadata, raw-resource fetch commands, `list`/`all`, separator-aware help/JSON arguments, and exact usage/unknown-topic errors. The red local-dispatch test, catalog/alias/settings JSON test, executable index/JSON/help/error/all-topics smokes, const-constructor/section-renderer simplification, simplified executable retest, all 149 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Hook namespace uninstall routing was pushed as `71f010bea`: `hooks uninstall [TARGET|--agent TARGET]`, legacy `uninstall-hooks`, and `hooks setup --uninstall` share one canonical alias map; omitted targets execute all 17 agent uninstallers and print the batch summary. Claude is correctly excluded because canonical injects Claude hooks through its wrapper rather than an agent definition. The red filtered/legacy/setup/batch parser test, isolated all-overrides/current-directory 17-agent executable batch smoke, shared-map simplification, all 147 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Codex coordinated uninstall was pushed as `d078a64ac`: direct uninstall removes 10 owned nested lifecycle/Feed hooks while preserving user groups, then restores the prior cmux-owned hooks feature line and removes marker-owned trust tables from `config.toml` while preserving user settings. Missing/invalid hooks JSON stops before TOML cleanup like canonical; missing TOML is non-fatal. Install/uninstall share the nested pruner, TOML normalization, and config resolver. The red parser/hooks/feature/trust restoration test, isolated executable install/remove/preservation/missing smoke, simplification review, focused retest, all 146 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Hermes Agent coordinated uninstall was pushed as `9d3165a32`: `hermes-agent`/`hermes` restores marker-owned YAML blocks (including inline-empty restore lines), preserves user YAML, removes exact owned event+command approvals from `shell-hooks-allowlist.json`, and preserves root metadata, user approvals, and nonconforming passthrough approvals. Missing/repeat behavior is canonical and config cleanup continues independently of allowlist presence. Install/uninstall share path and allowlist parse/root validation seams. The red parser/YAML-restore/exact-approval test, isolated executable two-file install/remove/repeat/missing/preservation smoke, shared-parser simplification, focused install regression test, simplified executable retest, all 145 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Kimi Code TOML uninstall was pushed as `875b48ad4`: direct uninstall removes every complete UUID-owned block and strips orphan begin markers without consuming following user tables, preserves unrelated TOML, reports repeat/missing states canonically, and shares the install path resolver. Kimi and Codex share one TOML normalization seam. The red parser/complete-block/orphan-marker test, isolated executable install/remove/repeat/orphan/missing smoke, TOML-normalization simplification, focused Codex regression test, corrected blank-separator executable retest, all 144 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Rovo Dev YAML uninstall was pushed as `c3d354cee`: `rovodev`/`rovo` direct uninstall removes every complete indentation-insensitive cmux marker block (and its preceding blank separator), preserves user YAML and dangling begin markers with following config, reports repeat/missing states canonically, and shares the install path resolver. Rovo and Hermes share one YAML normalization seam. The red parser/complete-block/dangling-marker test, isolated executable install/remove/repeat/dangling/missing smoke, YAML-normalization simplification, focused Hermes regression test, simplified executable retest, all 143 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Antigravity named-group uninstall was pushed as `9cce0f6ab`: `antigravity`/`agy` direct uninstall removes the reserved top-level `cmux` group only when it recursively contains a token-owned command, preserves all other root groups, refuses mention-only reserved groups with a zero-removal result, and matches canonical missing/malformed/non-object handling. Install/uninstall share one path resolver and the common home resolver. The red parser/owned-group/mention-only test, isolated executable install/remove/repeat/unowned/malformed/missing smoke, home-resolution simplification, simplified executable retest, all 142 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Flat-JSON hook uninstall for Kiro and Cursor was pushed as `2c18c303f`: direct uninstall prunes owned lifecycle, Feed, and stale-event entries across the hooks object, drops only emptied events, preserves user entries, scalar hooks, version/root metadata, and mention-only commands, and treats missing/invalid configs as canonical no-ops. Install shares the token-safe flat-entry pruner; nested and flat uninstall share one valid-existing-JSON read boundary. The red parser/preservation/stale-event/false-positive test, isolated executable install/uninstall/preservation/missing/invalid smokes for both agents, shared-reader simplification, simplified Kiro retest, all 141 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Nested-JSON hook uninstall for Gemini, Grok, Copilot, CodeBuddy, Factory, and Qoder was pushed as `09ba7de6c`: direct uninstall prunes owned lifecycle, Feed, and stale-event commands across the whole hooks object, drops only emptied groups/events, preserves mixed user commands, unknown group shapes, scalar hooks, and unrelated root fields, and treats missing/invalid configs as canonical no-ops. Install shares the same in-place `retain_mut` ownership pruning, and token-boundary executable matching avoids deleting user commands that merely mention `cmux hooks`. The red parser/mixed-group/stale-event/false-positive tests, isolated executable install/uninstall/preservation smokes for all six agents plus invalid JSON, simplified Qoder retest, all 140 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- OpenCode hook uninstall was pushed as `c0fd431a0`: global uninstall marker-safely removes the lifecycle session plugin, removes all canonical/legacy registration spellings while preserving user JSON/plugins, and scans both global and current-project Feed plugins; `--project` skips session/registration cleanup while matching canonical's two-path Feed scan. Missing and unmarked files retain canonical no-op/refusal/skip behavior, and concrete help advertises both scopes. The red scope/registration test, isolated executable global/project/install/remove/missing/mixed-ownership smokes, shared path/marker-state simplification, all 139 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Marker-owned Pi/OMP/Amp hook uninstall was pushed as `b056bba38`: direct `hooks AGENT uninstall` removes only a marker-owned extension/plugin, reports already-missing files idempotently, refuses while preserving an unmarked user file, and is advertised by concrete hooks help. All three agents share one read/ownership/remove state machine, and Amp install/uninstall share one config resolver. The red parser/ownership test, isolated executable install/remove/missing/refusal smokes, simplification review, all 138 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Codex hook installation was pushed as `90b1874e5`: direct/setup honors `CODEX_HOME`, requires an existing Codex directory, preserves user JSON/groups, and installs three lifecycle plus seven non-blocking 5-second Feed groups. It enables `[features] hooks = true` across table, dotted, missing, and replaced-user-setting forms with reversible ownership markers; appends 10 `hooks.state` trust tables keyed by normalized source/event/group/handler; and computes canonical SHA-256 identities from sorted event/matcher/handler JSON. The red JSON/feature/dotted/trust/idempotence test, shared feature-block simplification, isolated executable preservation/setup/missing-directory/reinstall smokes, all 137 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Kimi Code TOML hook installation was pushed as `c1d8347ef`: direct/setup `kimi` writes `config.toml` under `KIMI_CODE_HOME` or `~/.kimi-code`, preserves user TOML, replaces exactly one UUID-marked block, removes orphaned begin markers without dropping following config, and emits seven 10-second lifecycle plus three 120-second Feed `[[hooks]]` tables. TOML basic strings escape quotes, slashes, whitespace, and control scalars. The red preservation/shape/idempotence/orphan/escaping test, canonical-only command-surface simplification, isolated executable direct/setup/preservation/reinstall smokes, all 136 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Hermes Agent YAML and allowlist installation was pushed as `e0a3f738d`: direct/setup `hermes-agent` and `hermes` aliases honor `HERMES_HOME`, require an existing Hermes directory, merge eight lifecycle plus four Feed hooks into `config.yaml`, coalesce duplicate event names, preserve user hooks, and round-trip inline-empty roots/events with base64 restoration markers. `shell-hooks-allowlist.json` preserves user/nonconforming approvals, adds all 12 owned event-command pairs with valid RFC 3339 UTC timestamps, rejects non-object roots, and is idempotent. The red YAML/coalescing/inline-restore/allowlist test, shared YAML-quoting simplification, isolated executable two-file/setup/missing-directory/reinstall smokes, all 135 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Rovo Dev YAML hook installation was pushed as `6ec0ed3f8`: direct/setup `rovodev` and `rovo` aliases update `~/.rovodev/config.yml`, preserve unrelated root/nested YAML and user hooks, insert only under the direct root `eventHooks.events` tree, replace complete cmux marker blocks idempotently, and leave dangling markers plus following YAML intact. The canonical three Rovo events route through the local lifecycle bridge. The red nested-tree/marker/idempotence test, allocation-free YAML-key simplification, isolated executable alias/setup/preservation/reinstall smokes, all 134 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Amp TypeScript plugin installation was pushed as `52b16a2e3`: direct/setup installs canonical `~/.config/amp/plugins/cmux-session.ts`, replaces only marker-owned content, refuses unmarked files, and is idempotent. Its bundled resource exactly matches canonical's Swift source after line-ending normalization and bundles with Amp's type-only package externalized. Pi, OMP, and Amp share one marker-owned file transaction, while native path-component construction keeps Windows output separators correct. The red path/source/parser test, shared-installer/native-path simplify/retest pass, isolated executable direct/setup/install/reinstall/ownership-refusal smokes, all 133 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- OMP TypeScript extension installation was pushed as `1a11c5114`: direct/setup installs canonical `extensions/cmux-omp-session.ts`, resolves `PI_CODING_AGENT_DIR` before `PI_CONFIG_DIR` (default `.omp`) under `HOME`, expands tilde/absolute overrides, replaces only marker-owned content, refuses unmarked files, and is idempotent. The bundled resource exactly matches canonical's Swift source after line-ending normalization and bundles with OMP's type-only package externalized. Pi and OMP share one marker-owned extension transaction. The red path/source/parser test, shared-installer simplification, isolated executable resolver/direct/setup/install/reinstall/ownership-refusal smokes, all 132 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Pi TypeScript extension installation was pushed as `348005a7d`: direct/setup installs canonical `extensions/cmux-session.ts` under `PI_CODING_AGENT_DIR` or `~/.pi/agent`, expands tilde overrides, replaces only marker-owned content, refuses unmarked files, and is idempotent. The single bundled resource exactly composes canonical's two Swift source parts after line-ending normalization and bundles successfully with Pi's type-only package externalized. The red path/source/parser test, relative-path simplification, isolated executable direct/setup/install/reinstall/ownership-refusal smoke, all 131 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- OpenCode plugin installation was pushed as `4f403b05b`: global direct/setup installs the canonical registered `cmux-session.js` lifecycle plugin plus bundled `cmux-feed.js`, honors `OPENCODE_CONFIG_DIR`, preserves unrelated root config and scalar/tuple plugin entries, deduplicates legacy session specs, and refuses to overwrite unmarked plugin files. `--project` installs only `.opencode/plugins/cmux-feed.js`. Both canonical JavaScript sources bundle successfully, and the extracted session resource exactly matches canonical's embedded source after line-ending normalization. The red registration/shape test, ownership-helper/predicate simplification, isolated executable global/project/install/reinstall/ownership-refusal smoke, all 130 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Antigravity named-group JSON hook installation was pushed as `1c7e46152`: direct/setup `antigravity` and `agy` aliases write canonical `~/.gemini/config/hooks.json`, preserve every unrelated root group, and replace only the owned `cmux` group idempotently. The group carries 10-second lifecycle commands plus 120-second wildcard PreToolUse/PostToolUse Feed bridges. The red preservation/shape test, isolated executable alias/setup/install/reinstall smoke, all 129 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Cursor flat JSON hook installation was pushed as `b5aa33038`: direct/setup aliases write canonical `~/.cursor/hooks.json`, enforce `version: 1`, preserve unrelated root fields and user hook entries, install prompt/stop/agent-response/shell lifecycle commands plus the `beforeShellExecution` Feed bridge, and replace owned entries idempotently. The red shape test caught and fixed same-event owned-entry deletion; the one-clean-per-event simplification, isolated executable install/reinstall smoke, all 128 CLI library + 14 binary tests, strict CLI Clippy, desktop all-target checking/test linking/build, all 1,337 web/webview tests, typecheck/build, formatting, and whitespace checks pass.
- Nested JSON hook installers were pushed as `0762fa09e` for Gemini, Grok, Copilot, CodeBuddy, Factory, and Qoder.
- Generated agent lifecycle hook routing was pushed as `1daeb3f73`: generated lifecycle commands now transform into the bounded Feed bridge rather than re-entering the installer.
- Kiro hook installation was pushed as `0d4f5a642`: `cmux hooks kiro install` and setup aliases write the canonical custom-agent `agents/cmux.json`, default fresh agents to `tools: ["*"]`, preserve user fields/tools/hooks, replace only cmux-owned entries idempotently, install 5-second lifecycle hooks and 120-second pre/post-tool Feed hooks, omit Cursor's `version`, and print the required activation guidance.
- Feed legacy TUI and automatic fallback were pushed as `3b1129e55`: the Bun-independent crossterm path owns alternate-screen/raw-mode/cursor cleanup, latest-first pending refresh, j/k navigation, numeric single/multi question selection, permission and plan modes, replan feedback, refresh, and quit; automatic mode falls back after OpenTUI errors.
- Feed legacy TUI and automatic fallback are implemented and ready to push: the Bun-independent crossterm path owns alternate-screen/raw-mode/cursor cleanup, latest-first pending refresh, j/k navigation, numeric single/multi question selection, permission once/always/all/bypass/deny, plan default/auto/manual/ultraplan/bypass/deny, replan feedback, refresh, and quit. Explicit `--legacy` and `CMUX_FEED_TUI_LEGACY=1` select it; automatic mode falls back after an OpenTUI error while forced `--opentui` remains fail-closed. Red action-family tests, the pure one-write render simplification, lifecycle cleanup fixes, all 122 CLI library + 14 binary tests, CLI build, strict Clippy with only the three known unrelated allowances, desktop all-target checking/test linking/build, all 1,337 desktop/webview tests, web typecheck/build, formatting, and whitespace checks pass. The tool harness cannot allocate an interactive Windows TTY, so runtime verification is action/render/lifecycle contract plus compilation rather than a live keystroke smoke.
- Feed OpenTUI launch parity was pushed as `933b5e27d`: `cmux feed tui` / `--opentui` now parse canonical flags, require an interactive terminal before side effects, discover Bun from override/PATH/home, embed and stage the canonical TypeScript source plus pinned `@opentui/core` 0.1.106 package, install only on version drift, resolve the shared socket/password contract, and launch with canonical alternate-screen environment and exit handling. Nested TUI help and errors are exact. Red parser/plan tests, the shared socket-resolution simplification, embedded-source Bun bundling, executable help/noninteractive/no-staging/unknown smokes, all 121 CLI library + 13 binary tests, CLI build, strict Clippy with only the three known unrelated allowances, desktop all-target checking/test linking/build, all 1,337 desktop/webview tests, web typecheck/build, formatting, and whitespace checks pass.
- Canonical `cmux feed clear [--yes|-y]` was pushed as `71430151f`: the Windows CLI routes Feed locally without a socket, uses `~/.cmuxterm/workstream.jsonl`, confirms destructive deletion unless forced, preserves history on abort, and prints canonical cleared/missing/aborted messages plus concrete Feed help. Red routing/filesystem tests, the colocated-test simplification, real executable abort/forced-delete/missing/help smokes, all 117 CLI library + 13 binary tests, CLI build, strict Clippy with only the three known unrelated allowances, desktop all-target checking/test linking/build, all 1,337 desktop/webview tests, web typecheck/build, formatting, and whitespace checks pass.
- Feed older-history paging was pushed as `90d876e3d`: the shared persistence layer now pages backward with canonical stable byte-offset cursors in 300-physical-row chunks; the Windows Feed store tracks the oldest loaded offset, deduplicates item IDs, rebuilds carried context, and treats missing/read-failed older pages as non-fatal. All Activity now renders every telemetry kind and exposes canonical load/loading controls only while older persisted data exists; Tauri replies and cross-window events carry the has-more state. Red persistence/state/UI tests, the shared empty-page/reply-state simplifications, all 116 `cmux-agent` tests, strict agent Clippy, all 1,337 desktop/webview tests, web typecheck/build, desktop all-target checking/test linking/build, strict desktop Clippy with existing unrelated allowances, formatting, and whitespace checks pass.
- Feed plan-interview question parity was pushed as `ce9cd6a05`: the Windows Feed view now serializes each item's carried canonical context, and pending Claude questions expose `Skip + plan immediately` when permission mode is plan (case-insensitive) or user/assistant/question/option text contains one of canonical's four interview clues. The CTA returns the exact `Skip interview and plan immediately` selection and shares an equal-width question-action layout with submit. Backend and frontend tests were red before implementation; after implementation and the centralized-clue/dedicated-layout simplification, the focused Feed tests, all 1,336 desktop/webview tests, web typecheck/build, desktop all-target checking/test linking/build, strict Clippy with existing unrelated lint allowances, formatting, and whitespace checks pass.
- Feed hook context projection was pushed as `6f03264d1`: `UserPromptSubmit` now carries its prompt in `tool_input` and canonical normalized `lastUserMessage`; hook events preserve bounded assistant preamble, permission mode, existing context aliases, plan summary, allowed prompts, and agent-specific tool summaries. JSON-string tool inputs are decoded safely, every projected string uses one whitespace/truncation seam, and absent context stays omitted. The focused test was red before implementation; after implementation and simplification all 115 CLI library + 13 binary tests, CLI build, Clippy with only the three known unrelated allowances, desktop all-target checking/linking/build, all 1,334 desktop/webview tests, web typecheck/build, formatting, and whitespace checks pass.
- Feed hook identity/path/result enrichment was pushed as `716822495`: missing agent session IDs now use canonical SHA-256 fallback identity from source, agent PID, payload workspace, nested cwd/workspace paths, and nested transcript path, so repeated events group stably while distinct transcripts do not collide. Ambient workspace still wins, payload workspace aliases fill gaps, nested notification/data/context paths and tool-input cwd aliases project correctly, and every string alias shares canonical trimming. Codex `PostToolUse` now prefers the response aliases and replaces output/error bodies with canonical bounded metadata/omission summaries, preventing large or sensitive tool output from entering Feed/persistence. Red stable-identity/path/sanitization tests, centralized string normalization, all 114 CLI library + 13 binary tests, CLI build, Clippy with only the three known unrelated allowances, desktop all-target checking/linking/build, all 1,334 desktop/webview tests, web typecheck/build, formatting, and whitespace checks pass.
- Feed agent-native decision adapters were pushed as `4977db88e`: Kiro permission replies now allow with `{}` or deny/fail closed with canonical stderr and exit 2; Hermes permissions/plans/questions emit block, empty, or context JSON; Antigravity permissions emit native allow/deny JSON; and generic permission/plan/question adapters preserve canonical PreToolUse approve/block fields, additional context, persistent-permission guidance, mode-specific plan instructions, empty/single/numbered answers, and skip-interview behavior. The runtime now owns stdout/stderr/exit status explicitly, while tests use a private convenience renderer so the public API stays minimal. Red adapter/exit/context tests, that API simplification, all 112 CLI library + 13 binary tests, CLI build, Clippy with only the three known unrelated allowances, desktop all-target checking/linking/build, all 1,334 desktop/webview tests, web typecheck/build, formatting, and whitespace checks pass.
- The core Feed hook bridge was pushed as `9ce054946`: canonical `cmux hooks feed --source AGENT [--event EVENT]` and hidden legacy `feed-hook` now read bounded JSON stdin, no-op safely outside a cmux surface or on malformed/oversized input/socket failure, project session/workspace/cwd/tool/request/PID fields into `feed.push`, preserve the canonical source/event classification tables, keep telemetry at zero wait, and block actionable approvals for up to 120 seconds. Resolved Claude permission, plan, multi-question, always/all suggestion, auto-accept, and skip-interview decisions render agent-native `hookSpecificOutput`; generic agent decisions use the PreToolUse bridge. Red routing/classifier/mapping/decision tests, a simplified single response-output path, all 108 CLI library + 13 binary tests, CLI build, desktop all-target checking/linking/build, all 1,334 desktop/webview tests, web typecheck/build, formatting, whitespace, executable outside-surface/alias/help smokes, and Clippy with only three pre-existing unrelated lint allowances pass. Remaining hook parity is richer source/transcript context projection and installer coverage beyond the already bundled Claude wrapper.
- Feed multi-question controls were pushed as `49c3ffb19`: pending question cards now render every numbered prompt/header/option description, distinguish multi-select questions, retain single- and multi-choice drafts locally, accept per-question free-form answers, let an active free-form answer override presets exactly like canonical, and send one ordered human-readable answer per answered question through a single gated `Submit All Answers` action. Resolved cards render disabled submitted state, and all-optionless question batches can submit an empty answer. Red reducer/render tests, the shared preset-selection simplification, all 39 related panel/sidebar tests, all 1,334 desktop/webview tests, web typecheck/build, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass. Canonical plan-interview detection/`Skip + plan immediately` remains a separate context-projection follow-up.
- Feed JSONL history persistence was pushed as `c04072d0f`: the Windows store now matches canonical `~/.cmuxterm/workstream.jsonl` append-only storage, restores the latest 300 physical rows oldest-first, silently skips malformed JSON, rebuilds per-workstream context chronologically, redacts sensitive JSON keys/environment assignments/home paths before disk writes, and uses the canonical 2,000-item in-memory cap. Persistence load/write failures stay non-fatal, and resolutions/expiry remain memory-only like canonical. The red/green persistence tests, simplification to a shared context-index rebuild, all 115 `cmux-agent` tests, strict `cmux-agent` Clippy, all 1,331 desktop/webview tests, web typecheck/build, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass. Older-history paging, Feed clear, and Feed TUI remain explicit follow-ups. A whole desktop dependency-graph Clippy attempt still stops on the pre-existing `cmux-terminal` `obfuscated_if_else` lint outside this slice.
- Live Feed ingestion/decision parity was pushed as `8c195e7ed`: advertised `feed.push`, `feed.list`, `feed.permission.reply`, `feed.question.reply`, and `feed.exit_plan.reply` methods reuse the canonical Rust `WorkstreamEvent` mapper, retain bounded in-memory items/context, validate blocking timeout/id rules, wake blocking pushes with canonical decision JSON, and emit live panel snapshots. The Feed panel loads/listens, filters canonical actionable/activity kinds, renders permission/plan/question cards, and resolves decisions through the native store. Focused red/green UI contracts, the timestamp/state simplification, all 112 `cmux-agent` tests, all 1,331 desktop/webview tests, web typecheck/build, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass. Executing the new desktop Feed tests remains blocked before the harness by the known `STATUS_ENTRYPOINT_NOT_FOUND` loader issue; Feed TUI remains an explicit follow-up.
- Feed beta availability and shell parity was pushed as `916f9e83a`: the canonical-off-by-default preference persists in the existing app settings store, Settings exposes the Feed toggle, native mode validation admits Feed only while enabled and falls back to Files when disabled, and the mode bar plus command palette update dynamically. The Feed panel has canonical Actionable/All Activity controls and empty states. Focused red/green tests, the state-read simplification, all 1,329 desktop/webview tests, web typecheck/build, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- Core `right-sidebar` remote-control parity was pushed as `f06539f7a`: the CLI matches canonical toggle/show/hide/focus/set/mode and shorthand validation, normalizes `vault` to raw `sessions`, supports workspace/window/ambient targeting plus `--no-focus`, prints JSON only for mode queries, and has concrete help. A managed native state stays synchronized with local React actions; socket changes emit a native event that drives visibility/mode/focus. Focused red/green tests, executable help/error probes, the complete `cmux-cli` suite (101 library + 13 binary tests), all 1,324 desktop-web tests, web typecheck/build, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- `refresh-surfaces` parity was pushed as `961029ea8`: the CLI calls advertised `surface.refresh_all` with concrete help/plain `OK`; the Windows backend emits `cmux://refresh-surfaces`; and Workspace converts that native event into the existing browser resize signal used by mounted surface layout/ResizeObserver/xterm-fit paths without remounting state. Focused red/green tests, the minimal event-bridge simplification, the complete `cmux-cli` suite (100 library + 12 binary tests), all 26 Workspace tests, web typecheck/build, help smoke, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- Notification creation/delivery parity was pushed as `8f20d08b1`: `notify` now maps canonical title/subtitle/body defaults plus workspace/surface/window and ambient selectors to advertised `notification.create`; the Windows backend resolves the selected target, records a stable unread notification, builds the real delivery plan, and sends it through `WindowsToastDelivery`. Test and control toasts now share one identity/effects/plan/delivery helper. Focused red/green tests, the shared-delivery simplification, all 104 core notification tests, the complete `cmux-cli` suite (100 library + 12 binary tests), help smoke, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- Notification navigation parity was pushed as `9693085bb`: `open-notification` and `jump-to-unread` now call advertised `notification.open` / `notification.jump_to_unread`, share one newest-unread-or-ID lookup that marks the row read, reuse existing workspace/surface selection primitives, return canonical notification/opened payloads, and print canonical `OK` handle summaries. Focused red/green tests, a shared workspace-ID lookup simplification, all 104 core notification tests, the complete `cmux-cli` suite (100 library + 12 binary tests), help/error probes, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- Notification mutation parity was pushed as `69c4530ae`: `dismiss-notification`, `mark-notification-read`, and `clear-notifications` now enforce canonical selector rules and call advertised `notification.dismiss`, `notification.mark_read`, and `notification.clear` methods backed by the existing Windows notification store. ID, all-read, workspace, surface, all, ambient-workspace, and clear-all behavior share one store lock/projection seam and print plain `OK`. Focused red/green tests, the shared-lock simplification, all 104 core notification tests, the complete `cmux-cli` suite (100 library + 11 binary tests), help/error probes, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- Notification read parity was pushed as `7d2166c20`: `list-notifications` now calls `notification.list`, reads the existing Windows notification store through a shared control helper, returns canonical snake_case JSON rows, and preserves canonical pipe-delimited plain text plus concrete help. Focused red/green tests, a read-only store seam simplification, the complete `cmux-cli` suite (99 library + 10 binary tests), help smoke, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- Window introspection parity was pushed as `cb9767d8a`: `list-windows` and `current-window` now call read-only `window.list` / `window.current` backend methods projected from the Windows session snapshot, preserve canonical JSON shapes, canonical selected/workspace-count plain text, and concrete help. Focused red/green tests, a direct snapshot-projection simplification pass, the complete `cmux-cli` suite (99 library + 9 binary tests), both help probes, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- `reload-config` parity was pushed as `488b52b22`: the strict no-argument CLI calls `config.reload`, prints plain `OK`, and has concrete help; the Windows control backend advertises the method and reuses the config watcher's disk parse, typed validation, raw-cache replacement, and `cmux://config-changed` broadcast path. Focused red/green tests, the shared reload-path simplification, a raw-cache regression test, the complete `cmux-cli` suite (99 library + 8 binary tests), strict help/error smoke probes, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- `trigger-flash` parity was pushed as `8dd3f744c`: the canonical CLI now maps workspace/surface/window and ambient selectors to `surface.trigger_flash`, the Windows backend advertises the method and emits a targeted native event, and Workspace validates that payload before reusing its existing pane-flash animation. Focused red/green tests, a direct bridge simplification pass, the complete `cmux-cli` suite (99 library + 7 binary tests), all 25 Workspace tests, web typecheck/build, help smoke, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- Terminal `clear-history` parity was pushed as `3a6ed4cbd`: the Alacritty grid can now discard retained scrollback without erasing the viewport; the Windows terminal session and control backend advertise/handle canonical `surface.clear_history`; and the CLI supports workspace/surface/window scope, ambient selectors, concrete help, and plain `OK`. Focused red/green tests, a simplification pass that shares terminal target validation with text capture, all 169 `cmux-terminal` tests, the complete `cmux-cli` suite (99 library + 6 binary tests), help smoke, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- `restore-session` parity was pushed as `bdf5ff3c9`: the CLI now strictly accepts no arguments, calls canonical `session.restore_previous`, prints plain `OK`, and exposes concrete help; the Windows backend advertises and handles the canonical method alongside its existing port aliases. Focused red/green tests, a minimal validator/alias simplification pass, the complete `cmux-cli` suite (99 library + 5 binary tests), strict error/help smoke probes, desktop all-target checking, desktop test-target linking, desktop build, formatting, and whitespace checks pass.
- `rename-window` compatibility parity was pushed as `5a716fe3b`: the canonical alias now reuses `workspace.rename`, the existing workspace selector/title parser, ambient workspace scope, and concrete help while preserving its alias-specific missing-title error. Focused red/green tests, a simplification pass that kept one shared rename parser, the complete `cmux-cli` suite (99 library + 4 binary tests), help smoke output, formatting, and whitespace checks pass.
- Legacy browser alias parity was pushed as `1c84480b9`: `browser-reload`, `get-url`, `focus-webview`, and `is-webview-focused` now bridge canonical `--panel` selectors to the existing browser RPC methods, inherit ambient workspace scope, expose concrete help, and preserve canonical plain URL/boolean/`OK` output. Focused red/green tests, the simplified direct mapping pass, the complete `cmux-cli` suite (99 library + 4 binary tests), all four help smoke probes, formatting, and whitespace checks pass.
- `cmux capabilities` parity was pushed as `58b4e9efc`: the previously unmapped user-facing command now calls the already-supported `system.capabilities` backend method and has concrete no-socket help. The focused red/green test, simplified system-command test grouping, complete `cmux-cli` suite (99 library + 3 binary tests), help smoke probe, formatting, and whitespace checks pass.
