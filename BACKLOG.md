...
- [x] A5 — Group collapse/expand (`session_set_group_collapsed`). `[M, deps: A4, headless]` — done 2026-07-07 (frontend-parity): pure op + command + chevron toggle button.
- [x] A6 — Inline rename (`session_rename_workspace`; empty clears custom_title). `[M, deps: A4, headless]` — done 2026-07-07 (frontend-parity): canonical setCustomTitle port (user source, trim, empty clears title+source); dbl-click editor (Enter/blur commit, Esc cancel).
- [x] A7 — Pin/unpin + pinned-ahead reorder (`session_set_workspace_pinned`). `[M, deps: A1,A4, headless]` — done 2026-07-07 (frontend-parity): canonical setPinned+reorderTabForPinnedState port (selection follows by id; grouped rows defer contiguity-normalize w/ the A9 gap); hover pin-toggle in the sidebar.
