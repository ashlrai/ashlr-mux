...
mod tests {
    ...
    #[derive(Default)]
    struct AttributeRecorder(Vec<String>);

    impl Handler for AttributeRecorder {
        fn terminal_attribute(&mut self, attr: Attr) {
            self.0.push(format!("{attr:?}"));
        }
    }

    #[test]
    fn parse_overline_attributes() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = AttributeRecorder::default();

        parser.advance(&mut handler, b"\x1b[53m\x1b[55m");

        assert_eq!(handler.0, ["Overline", "CancelOverline"]);
    }
