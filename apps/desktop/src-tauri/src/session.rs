...
const MAX_MANUALLY_RESTORED_WINDOWS: usize = 12;

fn nonblank(value: Option<&str>) -> bool {
    value.is_some_and(|value| !value.trim().is_empty())
}

fn has_rows<T>(rows: Option<&Vec<T>>) -> bool {
...
fn crash_storage_roots() -> Vec<PathBuf> {
...
fn is_crash_storage_path(path: &str, roots: &[Vec<String>]) -> bool {
...
fn is_crash_diagnostic_workspace(
    workspace: &SessionWorkspaceSnapshot,
    roots: &[Vec<String>],
) -> bool {
...
fn readopt_additive_workspace_ids(
    current: &AppSessionSnapshot,
