...
#[test]
fn production_helpers_and_socket_routes_use_typed_change_gated_results_without_bypasses() {
    ...
    for signature in [
        ...
        assert!(
            body.contains("state.transact_result_if_changed(app,"),
            "{signature}"
        );
    ...
    for (signature, failure_message) in [
        ("fn surface_move(", "Failed to move surface"),
        ("fn surface_reorder(", "Failed to reorder surface"),
    ] {
        let body = function_source(socket, signature);
        assert!(
            body.contains("PaneTopologyControlError::Operation"),
            "{signature}"
        );
        assert!(
        ...
        assert!(body.contains("\"internal_error\""), "{signature}");
        assert!(body.contains(failure_message), "{signature}");
        for key in [
            "window_id",
            "workspace_id",
            "pane_id",
            ...
            assert!(body.contains(key), "{signature} lost {key} success field");
        }
    }
    let join = function_source(socket, "fn pane_join(");
    assert!(join.contains("surface_move(app, &move_params)"));
    assert!(!join.contains("move_surface_for_control("));
}
