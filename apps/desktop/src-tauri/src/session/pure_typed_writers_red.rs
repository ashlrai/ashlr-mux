...
fn production_writers_and_callers_use_typed_fallible_paths_with_durable_diff() {
    ...
    let zoom_compact: String = zoom
        ...
        .collect();
    assert!(zoom.contains("Result<AppSessionSnapshot, String>"));
    assert!(zoom.contains("transact_result_always(") || zoom.contains("transact_pane_topology("));
    assert!(zoom.contains("unable to set browser zoom for pane"));
    assert!(
        !zoom_compact.contains(".snapshot.lock()") && !zoom.contains("notify_session_changed(")
    );

    let rename = function_source(session, "pub fn session_rename_workspace(");
    assert!(rename.contains("Result<AppSessionSnapshot, String>"));
    assert!(rename.contains("transact_snapshot_if_changed("));
    assert!(!rename.contains("snapshot.lock()") && !rename.contains("notify_session_changed("));

    let agent = function_source(session, "pub(crate) fn record_started_agent_session(");
    assert!(agent.contains("Result<bool, String>"));
    assert!(agent.contains("transact_value_if_changed("));
    assert!(!agent.contains("snapshot.lock()") && !agent.contains("notify_session_changed("));

    let optional = function_source(session, "fn mutate_optional_snapshot_for_control");
    assert!(optional.contains("Result<Option<AppSessionSnapshot>, String>"));
    assert!(optional.contains("transact_value_if_changed("));
    for writer in OPTIONAL_WRITERS {
        let body = function_source(session, &format!("pub(crate) fn {}(", writer.helper()));
        assert!(body.contains("Result<Option<AppSessionSnapshot>, String>"));
        assert_eq!(
            body.matches("mutate_optional_snapshot_for_control(")
                .count(),
            1
        );
    ...
    for (public, helper, message) in [
        (
            "session_open_markdown_file",
            "open_markdown_file_in_panel(",
            "unable to open markdown file in pane",
        ),
        (
            "session_open_file",
            "open_file_in_panel(",
            "unable to open file in pane",
        ),
    ] {
        let body = function_source(session, &format!("pub fn {public}("));
    ...
    for (route, helper) in [
        ("fn surface_open_markdown(", "open_markdown_file_in_panel("),
        ("fn surface_open_file(", "open_file_in_panel("),
        ("fn sidebar_open(", "open_custom_sidebar_in_panel("),
    ] {
        let body = function_source(socket, route);
        assert!(body.contains(helper));
        assert!(body.contains("Ok(Some(snapshot))"));
    ...
    let agent_caller = include_str!("../agent_session.rs");
    let agent_offset = agent_caller.find("record_started_agent_session(").unwrap();
    let agent_context = &agent_caller
        [agent_offset.saturating_sub(80)..(agent_offset + 300).min(agent_caller.len())];
    assert!(
        agent_context.contains('?')
            || agent_context.contains("Err(")
            || agent_context.contains("eprintln!")
    );
    let markdown = include_str!("../markdown.rs");
    let markdown_offset = markdown.find("open_markdown_file_in_panel(").unwrap();
    let markdown_context =
        &markdown[markdown_offset.saturating_sub(80)..(markdown_offset + 300).min(markdown.len())];
    ...
    for public in ["session_set_browser_zoom", "session_rename_workspace"] {
        assert!(web.contains(&format!(".invoke<AppSessionSnapshot>(\"{public}\"")));
        assert!(web.contains(&format!("{public} failed")));
    }
}
