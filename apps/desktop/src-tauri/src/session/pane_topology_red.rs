...
#[test]
fn production_helpers_and_socket_routes_use_typed_publication_errors_without_bypasses() {
    ...
    for signature in [
        ...
        assert!(
            body.contains("state.transact_pane_topology(app,"),
            "{signature}"
        );
    ...
    for (signature, variants, operation_codes) in [
        (
            "fn surface_split_off(",
            &["WouldEmptySourcePane", "SurfaceNotFound"][..],
        ...
        (
            ...
            &["invalid_params", "not_found", "invalid_state"][..],
        ),
        (
            "fn pane_break(",
            &["WorkspaceNotFound", "SurfaceNotFound", "DetachFailed"][..],
        ...
        for variant in variants {
            assert!(body.contains(variant), "{signature} lost {variant} mapping");
        }
        for code in operation_codes {
            assert!(body.contains(code), "{signature} lost {code} mapping");
        }
        assert!(
        ...
        if let Some(focus) = body.find("window.set_focus()") {
            assert!(
                focus > publication,
