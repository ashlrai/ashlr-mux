...
fn assert_transactional_helper(body: &str) {
    assert!(body.contains("transact_"));
    assert!(!body.contains("snapshot.lock()"));
    assert!(!body.contains("notify_session_changed("));
    assert!(!body.contains("current_session_snapshot("));
    assert!(!body.contains("start_panel_browser_proxy_for_control("));
}

#[test]
fn production_four_helpers_are_typed_transactional_and_public_duplicates_delegate() {
    let session = include_str!("../session.rs");
    assert!(session.contains("trait BrowserProxyEffects"));
    assert!(session.contains("BrowserProxyLease"));
    assert!(session.contains("rollback"));
    assert!(session.contains("commit"));
    let open = function_source(session, "pub(crate) fn open_browser_url_in_panel(");
    assert!(open.contains("Result<Option<AppSessionSnapshot>, String>"));
    assert_transactional_helper(open);

    let split = function_source(session, "pub(crate) fn split_browser_for_control(");
    assert!(split.contains("Result<AppSessionSnapshot, BrowserPanelCreateError>"));
    assert_transactional_helper(split);
    assert!(!split.contains("next_panel.fetch_add"));

    let new_workspace =
        function_source(session, "pub(crate) fn new_browser_workspace_for_control(");
    assert!(new_workspace.contains("Result<AppSessionSnapshot, String>"));
    assert_transactional_helper(new_workspace);
    assert!(!new_workspace.contains("next_panel.fetch_add"));

    let reopen = function_source(
        session,
        "pub(crate) fn reopen_closed_browser_tab_for_control(",
    );
    assert!(reopen.contains("Result<AppSessionSnapshot, String>"));
    assert_transactional_helper(reopen);
    assert!(!reopen.contains("next_panel.fetch_add"));
    assert!(!reopen.contains("history.pop()"));

    for (public, helper) in [
        (
        ...
        (
            ...
            "new_browser_workspace_for_control(",
        ),
        (
            "pub fn session_reopen_closed_browser_tab(",
            "reopen_closed_browser_tab_for_control(",
        ),
    ] {
        let body = function_source(session, public);
    ...
    let open_public = function_source(session, "pub fn session_open_browser_url(");
    assert!(open_public.contains('?'));
    assert!(open_public.contains("unable to open browser in pane"));
}

#[test]
fn socket_routes_distinguish_domain_misses_from_publication_failures() {
    let socket = include_str!("../control_socket.rs");
    for route in ["fn surface_open_browser(", "fn browser_navigate("] {
    ...
    for route in ["fn surface_split_browser(", "fn browser_open_split("] {
        let body = function_source(socket, route);
        assert!(body.contains("BrowserPanelCreateError::NotFound"));
        assert!(body.contains("BrowserPanelCreateError::Publication"));
        assert!(body.contains("\"not_found\""));
        assert!(body.contains("\"internal\""));
    }
    for route in ["fn workspace_create_browser(", "fn browser_reopen_closed("] {
        let body = function_source(socket, route);
        assert!(body.contains("Ok(snapshot)"));
        assert!(body.contains("Err(message)"));
        assert!(body.contains("\"internal\""));
    }
}
