...
#[test]
fn production_has_collision_safe_pending_cleanup_lease_ownership() {
    let browser = include_str!("../browser.rs");
    for required in [
        "pending_cleanup_children",
        "BrowserPendingCleanupId",
        "pending_cleanup_id",
        "BROWSER_RUNTIME_LOCK_ORDER",
    ] {
        assert!(
            browser.contains(required),
            "missing pending ownership: {required}"
        );
    }

    let state = source_item(browser, "pub struct BrowserWebviewState");
    assert!(state.contains("pending_cleanup_children"));
    assert!(state.contains("Mutex"));

    let entry = source_item(browser, "pub(crate) struct BrowserPanelRuntimeEntry");
    assert!(entry.contains("pending_cleanup"));

    let cleanup = source_item(browser, "fn cleanup_built_browser_child_after_failure(");
    assert!(cleanup.contains("pending_cleanup_children"));
    assert!(cleanup.contains("entry("));
    assert!(cleanup.contains("pending_cleanup_id"));
    assert!(!cleanup.contains("webviews.insert"));

    let reservation = source_item(browser, "struct BrowserPanelMutationReservation");
    assert!(reservation.contains("pending_cleanup_id"));
    ...
    for lock in order {
        let position = detach[cursor..]
            .find(lock)
            .unwrap_or_else(|| panic!("detach missing ordered lock: {lock}"))
            + cursor;
        cursor = position + lock.len();
    }
    assert!(!detach.contains(".close("));

    let rollback = source_item(browser, "rollback_browser_panels_for_control");
    assert!(rollback.contains("pending_cleanup_children"));
    assert!(rollback.contains("contains_key"));
    assert!(rollback.find("contains_key").unwrap() < rollback.find("insert(").unwrap());

    let finalize = source_item(browser, "finalize_browser_panels_for_control");
    assert!(finalize.contains("pending_cleanup"));
    assert!(finalize.contains("retry"));
    assert!(finalize.contains(".close("));
    assert!(finalize.find("drop(").unwrap() < finalize.find(".close(").unwrap());

    let compensation = source_item(
        browser,
        "fn cleanup_tracked_browser_child_for_compensation(",
    );
    assert!(compensation.contains("pending_cleanup_id"));
    assert!(compensation.contains("pending_cleanup_children"));
    assert!(!compensation.contains("state.webviews"));
    assert!(compensation.contains("remove(&pending_cleanup_id)"));
    assert!(!compensation.contains("pop("));
    assert!(!compensation.contains("pop_last("));
}
