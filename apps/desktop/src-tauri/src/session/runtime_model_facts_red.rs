...
#[test]
fn production_policies_routes_and_background_producers_are_fallible_without_bypasses() {
    let session = include_str!("../session.rs");
    for kind in CHANGE_GATED {
        let body = function_source(session, &format!("pub(crate) fn {}(", kind.helper()));
        assert!(
        ...
        assert!(
            body.contains("transact_snapshot_if_changed(app,"),
            "{}",
            kind.helper()
        );
        ...
        if matches!(
            kind,
            WriterKind::PanelTty | WriterKind::PanelShell | WriterKind::SetAgentPid
        ) {
            assert!(
                body.contains("current_unix_timestamp_seconds()"),
                "{}",
                kind.helper()
            );
        }
    }
    let kind = function_source(session, "pub(crate) fn set_surface_kind_for_control(");
    assert!(kind.contains("Result<AppSessionSnapshot, String>"));
    assert!(kind.contains("transact_snapshot_always(app,"));
    assert!(!kind.contains("snapshot.lock()") && !kind.contains("notify_session_changed("));

    let socket = include_str!("../control_socket.rs");
    for (route, helper, success) in [
        (
            "fn surface_set_kind(",
            "set_surface_kind_for_control(",
            "surface_list_from_params(",
        ),
        (
            "fn surface_report_tty(",
            "set_panel_tty_for_control(",
            "surface_list_from_params(",
        ),
        (
        ...
        (
            ...
            "set_panel_listening_ports_for_control(",
            "surface_list_from_params(",
        ),
        (
            "fn workspace_set_agent_pid(",
            "set_workspace_agent_pid_for_control(",
            "workspace_current(",
        ),
        (
            "fn workspace_clear_agent_pid(",
            "clear_workspace_agent_pid_for_control(",
            "workspace_current(",
        ),
        (
            "fn workspace_report_pr(",
            "set_workspace_panel_pull_request_for_control(",
            "workspace_current(",
        ),
        (
            "fn workspace_clear_pr(",
            "clear_workspace_panel_pull_request_for_control(",
            "workspace_current(",
        ),
    ] {
        let body = function_source(socket, route);
        assert!(body.contains(helper), "{route}");
        assert!(body.contains(success), "{route}");
        assert!(body.contains("Err(message)"), "{route}");
        assert!(body.contains("\"internal\""), "{route}");
    }

    let clear = function_source(socket, "fn workspace_clear_agent_pid(");
    assert!(
        clear
            .find("clear_workspace_agent_pid_for_control(")
            .unwrap()
            < clear.find("refresh_workspace_agent_ports(").unwrap()
    );

    for (source, helper) in [
        (
            include_str!("../terminal.rs"),
            "set_process_title_for_panel(",
        ),
        (
            include_str!("../terminal.rs"),
            "set_panel_listening_ports_for_panel(",
        ),
        (
            include_str!("../agent_session.rs"),
            "set_workspace_agent_listening_ports_for_control(",
        ),
        (
            include_str!("../workspace_pull_requests.rs"),
            "set_workspace_git_facts_for_control(",
        ),
    ] {
        let offset = source.find(helper).unwrap();
        let context =
            &source[offset.saturating_sub(80)..(offset + helper.len() + 200).min(source.len())];
        assert!(
            context.contains('?') || context.contains("let _ =") || context.contains("match "),
            "{helper}"
        );
    }
}
