...
- Windows behavior: `eb33086f7ab5a0f423061e069a07b6b2d5f2b638`
- Latest Windows code checkpoint: `eb33086f7ab5a0f423061e069a07b6b2d5f2b638`
...
comparison remains 38 identical and 30 delta cases because independent native
...
1. The window-lifecycle transport and public UUID-routing blockers are closed,
   but 30 of 68 cases still differ. The largest actionable cluster is now
   resume-binding response/state payloads. Window-family residuals are mostly
   native key/fallback choice, AppKit's asynchronous closed-window rows,
   native window ordering, and platform paths; preserve those distinctions
   instead of changing Windows behavior merely to reduce the case count.
...
Move to the resume-binding response/state cluster, which affects eleven strict
cases and is independent of native window behavior. Start with
`surface_resume.roundtrip_set_get_clear_get`, compare the exact binding and
clear payloads, and repair the smallest shared serializer/state boundary.
Then cover alias precedence, checkpoint guards, source/auto-resume policy, and
cross-window lookup with the same implementation. Run a resume-only prefix
twice around simplification, then the 66-case no-restart prefix and the full
68-case matrix. Keep persistence/restart behavior separate if its mismatch is
not caused by the serializer.
