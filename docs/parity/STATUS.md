...
- Windows behavior: `2e280b9fb9b971ce0182b30bcfa7706c14c0b2e5`
- Latest Windows code checkpoint: `2e280b9fb9b971ce0182b30bcfa7706c14c0b2e5`
...
2. Four differential-remediation unit tests fail unchanged at both pushed
   ...
   desktop library gate is 1,042 passed, 1 ignored, and 4 explicitly excluded;
...
- Canonical initial window-event construction now lives in the 226-line
  ...
  publication executor share the same prepared window identities.
...
Use the valid 68-case comparison to repair the window-list state seam shared by
create, close, focus, and the CLI wrappers. First separate genuine Windows
platform equivalences (home-directory strings and synchronous native teardown)
from observable identity, ordering, key, and visibility mismatches. Then fix
the smallest shared model boundary that removes the most state/multiwindow
lanes; do not emulate canonical zombie-window timing merely to satisfy the
harness. Rerun the focused create/close/focus cases twice, simplify, rerun
twice, and finish with all 68 cases. Treat resume-binding payloads as the next
independent slice rather than mixing them into window state.
