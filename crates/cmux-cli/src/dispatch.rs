...
fn mapped_subcommand_usage(command: &str) -> Option<&'static str> {
    match command {
        ...
        "hooks" => Some(
            "Usage:\n  cmux hooks feed --source AGENT [--event EVENT]\n  cmux hooks AGENT install [--yes|-y]\n  cmux hooks opencode install [--project] [--yes|-y]\n  cmux hooks setup --agent AGENT [--yes|-y]\n\nBridges agent events into Feed or installs hooks for Claude, Kiro, Gemini, Grok, Copilot, CodeBuddy, Factory, Qoder, Cursor, Antigravity, OpenCode, Pi, OMP, Amp, Rovo Dev, and Hermes Agent.",
        ),
        _ => None,
    }
}
